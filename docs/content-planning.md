...
랜딩페이지, 마케팅 카피, 이메일, 문서, 튜토리얼, 업데이트를 포함한 모든 공개 콘텐츠는 제목, 초안, 영상보다 기획서가 먼저입니다. 콘텐츠 단위는 파일이 아니라 독자 결과 하나입니다. 각 단위는 `docs/content-plans/<slug>.md`에 기획서를 두고 `approved`가 된 뒤 제작합니다. 기획서는 독자를 제품으로 데려오기 위한 설명서가 아니라, 독자가 이미 겪는 문제에서 출발해 무엇을 이해하고 시도해야 하는지 정하는 문서입니다.
...
- v0.3, 2026-08-24: 모든 공개 콘텐츠와 마케팅 화면에 적용하고 파일이 아닌 독자 결과를 콘텐츠 단위로 정함.

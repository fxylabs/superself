...
| R11 | Where is a judgment's parent recorded? | **On the `entity.covered` event, as `payload.judgedUnder`**, written by the one coverage writer whenever the covered record is a milestone. No new event type: the design forbids one, and the vocabulary proof in `docs.test.mjs` would have to grow for it |
...
| 16 | `work link <w> --milestone <live milestone under a dropped objective>` | refused: the checkpoint is closed because its objective is |
...
`eventRecord` (`logfile.ts`) reads a bare `requirement`, `milestone` or
`objective` on a payload as the record the whole event is about, before it
ever falls through to `entity`. Both of #417 part (b)'s new payload fields
carried an id under one of those bare names, which misfiled the event on the
gap or parent named rather than on the record the event is actually about.
Fixed by nesting the retarget's gap under `payload.gap` and naming the
coverage claim's parent `payload.judgedUnder` — neither name `eventRecord`'s
generic priority reads — without touching that priority, which other event
types still depend on.

| # | Case | Outcome |
|---|---|---|
| 59 | `state show <checkpoint> --history` and `state show <objective> --history`, after `milestone met` names that objective as the judgment's parent | the claim reads in the checkpoint's own history; the objective's history carries no trace of it |
| 60 | `state show <plan> --history` and `state show <gap> --history`, after `work revise` retargets the plan onto a gap — checked for both the `--milestone` and `--objective` alternatives | the revision reads in the plan's own history; the destination gap's history carries no trace of it |

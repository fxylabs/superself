...
- Problem-narrative content loop playbook (generalized from superself's state-not-memory piece, 2026-08-07): (1) Pick each topic from measured demand — search the problem phrasing users actually type, classify the existing solution landscape, and aim the piece where the demand stream is largest but the standing answer is wrong or missing. (2) The operator dictates the thesis in their own words before drafting; the draft preserves that voice, opens by conceding what existing tools do well, and passes an AI-tell audit (no em-dashes, at most one contrast construction, varied sentence lengths, no stock AI vocabulary). (3) Visuals are real product output only: terminal recordings captured from actual CLI runs in an isolated scratch workspace (vhs), styled with the site's design tokens. (4) The product site publishes first and owns the canonical URL; dev.to carries the cross-post with canonical_url set, and all pieces join one series per product so each piece feeds the next. (5) Distribute as an answer where problem awareness already exists: read the full target thread including every comment before drafting, credit the target's design accurately, build on a limitation its author stated, and disclose authorship. (6) Read per-piece signals at the weekly review from the daily adoption snapshot plus channel counters, and end every review in a recorded decision — strategy adjustment or explicit no-change.
<!-- superself:end -->

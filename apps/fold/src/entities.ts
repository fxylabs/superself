...
function collectCoverage(fold: EntityFold, event: SelfEvent): void
{
    ...
    fold.coverage.push({
        ...
        claim: {
            ...
            objective: str(event.payload.judgedUnder)
        }
    });
}
...
// Read from the nested `gap` a revision writes (#455) — a bare top-level
// `objective` or `milestone` is what `eventRecord` reads as the record the
// event is about, so the revision's own target has to sit under a key that
// generic reader never looks at.
function statedTarget(payload: Record<string, unknown>): PlanTarget | undefined
{
    const gap = payload.gap as Record<string, unknown> | undefined;
    const objective = str(gap?.objective);
    const milestone = str(gap?.milestone);

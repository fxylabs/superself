...
function cmdWorkRevise({ values, positionals }: CommandInput<typeof REVISE_OPTIONS>): CommandOutput
{
    ...
    const why = required(values.why);
    recordEvents(ctx, [
        // The retarget's gap nests under its own key rather than sitting bare
        // on the payload (#455): `eventRecord` reads a bare `milestone` or
        // `objective` as the record the whole event is about, which would
        // file this revision under the gap it moves to instead of the work
        // unit whose plan changed.
        makeEvent(owner, "entity.revised", strip({ entity: entity.id, text, why, gap: moved?.gap })),

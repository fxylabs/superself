...
const BLOCK_BODY: readonly string[] = [
        ...
        "- Before trusting the direction graph, read it: `self objective check` names",
        "  work whose every contribution is to an outcome that is over, a successor",
        "  checkpoint with no live work, a checkpoint dated past its objective, a coverage",
        "  judgment or an assumption to re-review, done work that is a candidate for an",
        "  uncovered criterion, work that states no disposition, and an objective whose",
        "  whole live workload is runbook occurrences. It changes nothing: every line it",
        "  prints is a command you run yourself, and it never relinks, covers, revises or",
        "  reclassifies anything. A candidate is information — nothing is paired to a",
        "  criterion by its wording, and maintenance is read off the run link and never",
        "  out of a record's text. Where a project a unit contributes to is not readable",
        "  here, the line says the target state was not checked rather than reporting all",
        "  clear. `self status` and `self context` carry the count.",

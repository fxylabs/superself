...
const BLOCK_BODY: readonly string[] = [
        ...
        "- Substantive work attaches to a work unit, and a session proposes one rather",
        "  than recording it: `self work propose \"<plan>\"`, which the user accepts with",
        "  `self work accept <id>`. Both `self work add` and `self work accept` write a",
        "  confirmed record and are refused where no person is at the terminal. Then",
        ...
        "- A unit states what it contributes to: `self work link <id> --objective <objective-id>`",
        "  (or `--milestone <id>`). Nothing forces it, and `self objective` lists the open",
        "  ones to pick from — an unattached unit is one nobody can tell apart from the pile.",
        ...
        "  named unit retires when the user accepts — records the new wording and carries the",
        ...
        "  behind the proposal, and hand the user the `self work accept <id>` line — an",

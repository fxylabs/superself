...
must(box, demo, ["project", "init", "--name", "demo", "--desc", "the document migration", "--no-connect"]);
must(box, other, ["project", "init", "--name", "other", "--desc", "the second log", "--no-connect"]);

must(box, demo, ["goal", "add", "every page answers through the gate"]);
must(box, demo, ["convention", "add", "a handler never asks which render this run is"]);
must(box, demo, ["decide", "the pages move in stage four", "--proposed"]);
must(box, demo, ["state", "add", "a note the search surface finds", "--exposure", "search"]);
const objective = idOf(must(box, demo, ["objective", "add", "the documents answer with blocks"]).out, "o");
const milestone = idOf(must(box, demo, ["milestone", "add", "the pages carry both renders",
    "--objective", objective, "--exit", "no page prints for itself"]).out, "m");

const running = workIdIn(must(box, demo, ["work", "add", "the context renders behind the gate"]).out);
must(box, demo, ["work", "start", running]);
must(box, demo, ["report", running, "the plain render runs the budget inside its own thunk"]);
must(box, demo, ["work", "started", running, "--pid", String(process.pid)]);

const held = workIdIn(must(box, demo, ["work", "add", "the show pages keep their lead lines"]).out);
must(box, demo, ["work", "start", held], { SUPERSELF_SESSION: "another-session", SUPERSELF_SESSION_PID: String(process.pid) });
must(box, demo, ["state", "place", held, "--scope", "other"]);
...
test("stage 4 cell 1: a piped `self context` is the markdown page, inside its budget", () =>
{
    const printed = lines(selfIn(box, demo, ["context"]));
...
test("stage 4 cell 2: records past the budget leave counted omissions that name the way back", () =>
{
    const own = budgetedWorkspace();
    const printed = must(own.box, own.demo, ["context"]).out;
...
test("stage 4 cell 3: an archived project says so first, then prints the page", () =>
{
    const own = archivedProject();
    const printed = lines(must(own.box, own.demo, ["context"]));
    assert.match(printed[0], /^project "demo" is archived \(/);
    assert.equal(printed[1], "# demo");
});

test("stage 4 cell 5: a piped `self context` from the workspace is one line per project", () =>
{
    const printed = lines(selfIn(box, ws, ["context"]));
...
test("stage 4 cell 6: a piped `self status` keeps its roll-up lines and the processes under them", () =>
{
    const printed = lines(selfIn(box, demo, ["status"]));
...
test("stage 4 cell 7: `self status --workspace` from outside every project is the overview", () =>
{
    const printed = lines(selfIn(box, box.root, ["status", "--workspace"]));
    assert.equal(printed.length, 2, printed.join("\n"));
...
test("stage 4 cell 8: `self work show` prints the held and elsewhere lines before the body", () =>
{
    const printed = lines(selfIn(box, demo, ["work", "show", held]));
...
test("stage 4 cell 9: a record past one page shows today's page and the pointer to the next", () =>
{
    const own = pagedRecord();
    const first = lines(must(own.box, own.demo, ["state", "show", own.id, "--history"]));
    ...
    const second = lines(must(own.box, own.demo, ["state", "show", own.id, "--history", "--page", "2"]));
...
test("stage 4 cell 10: `objective show` and `milestone show` print their pages", () =>
{
    const shown = lines(selfIn(box, demo, ["objective", "show", objective]));
    assert.equal(shown[0], `# ${objective} — the documents answer with blocks`);
    assert.ok(shown.includes("## Exit criteria"), shown.join("\n"));
    const checkpoint = lines(selfIn(box, demo, ["milestone", "show", milestone]));
...
test("stage 4 cell 11: a piped `self state` prints its rows and then how many live records there are", () =>
{
    const printed = lines(selfIn(box, demo, ["state"]));
...
test("stage 4 cell 11: one live record is counted in the singular", () =>
{
    const own = machineWithDemo();
    must(own.box, own.demo, ["decide", "the only record here"]);
    assert.equal(lines(must(own.box, own.demo, ["state"])).at(-1), "1 live entity");
});

test("stage 4 cell 12: a project with nothing live keeps its wording and gains no size line", () =>
{
    const own = machineWithDemo();
    assert.deepEqual(lines(must(own.box, own.demo, ["state"])), ["no live entities"]);
});

/* ── the diagnostics that must work where nothing is set up ────────── */

test("stage 4 cell 14: `self setup` answers on a machine with no workspace at all", () =>
{
    const bare = machine();
    assert.deepEqual(lines(selfIn(bare, bare.root, ["setup"])), [
...
test("stage 4 cell 17: `--pretty --plain` together is refused in the same words on every page", () =>
{
    for (const args of [["context"], ["status"], ["work"]])
    {
        const refused = selfIn(box, demo, [...args, "--pretty", "--plain"]);
...
function machineWithDemo()
{
    ...
    must(own, project, ["project", "init", "--name", "demo", "--desc", "a scratch project", "--no-connect"]);
...
function budgetedWorkspace()
{
    ...
    {
        must(own.box, own.demo, ["convention", "add", `rule ${name} ${"x".repeat(5_000)}`]);
    }
    must(own.box, own.demo, ["decide", "small enough to be omitted"]);
    return own;
}
...
{
    const own = machineWithDemo();
    must(own.box, own.ws, ["project", "archive", "demo", "--why", "set aside for the quarter"]);
    return own;
}
...
function pagedRecord()
{
    const own = machineWithDemo();
    const id = idOf(must(own.box, own.demo, ["state", "add", "a record placed again and again",
        "--exposure", "search"]).out, "e");
    for (let priority = 1; priority <= 11; priority++)
    {
        must(own.box, own.demo, ["state", "place", id, "--priority", String(priority), "--exposure", "search"]);
    }
    return { ...own, id };
}

...
async function attach(args, at = demo)
{
    const work = await unit(at);
    return { work, ...await selfIn(box, at, ["report", work, "attached", ...args]) };
}
...
async function entryOf(name, entries, flags = [])
{
    const { work, code, out } = await attach(["--artifact", tree(name, entries), ...flags]);
...
async function freshMachine()
{
    const other = machine();
    delete other.env.CI;
    const places = await demoWorkspace(other);
    return { box: other, ...places };
}

const reading = await freshMachine();
...
const readingBundle = await readingFixture();
...
async function hostileBundle(place, mutate)
{
...
test("cell 1: a directory attaches as one artifact whose members are sorted, forward-slashed paths", async () =>
{
    const root = tree("dist", { "index.html": "<h1>hi</h1>", "assets/app.js": "js", "assets/style.css": "css" });
    const { work, code, out } = await attach(["--artifact", root]);
    assert.equal(code, 0, out);
...
test("cell 2: an empty directory is refused, and stages nothing", async () =>
{
    const before = storeTree();
    const { work, code, out } = await attach(["--artifact", tree("empty", {})]);
...
test("cell 3: a directory holding only .git is refused as empty, and the message says .git is not copied", async () =>
{
    const { code, out } = await attach(["--artifact", tree("repo", { ".git/HEAD": "ref: refs/heads/main\n" })]);
    assert.equal(code, 1, out);
    assert.match(out, /holds no files to attach/);
    assert.match(out, /`\.git` is never one of them/);
});

test("cell 4: an empty subdirectory is neither a member nor created in the store, and nested files keep their path", async () =>
{
...
test("cell 5: a symlink inside the tree refuses the bundle, whether its target is inside or outside", async () =>
{
    ...
    for (const [label, target] of [["inside", "a.txt"], ["outside", outside]])
    {
        ...
        const before = storeTree();
        const { code, out } = await attach(["--artifact", root]);
...
test("cell 6: a fifo inside the tree refuses the bundle by the same rule", async () =>
{
...
test("cell 7: --artifact on a symlink to a directory ingests that directory's files", async () =>
{
    const root = tree("dist", { "index.html": "hi", "assets/app.js": "js" });
    const link = join(dirname(root), "link-to-dist");
    symlinkSync(root, link);
    const { work, code, out } = await attach(["--artifact", link]);
    assert.equal(code, 0, out);
...
test("cell 8: a member name holding a newline refuses the bundle, naming the path", async () =>
{
...
test("cell 9: names holding non-ASCII text and an emoji record verbatim, and open resolves", async () =>
{
    ...
    const opened = await must(box, demo, ["artifact", "open", meta.id], { SUPERSELF_SESSION: "s" });
...
test("cell 11: two member paths colliding under case folding are refused at plan time, on every filesystem", async () =>
{
...
test("cell 51: two member paths colliding under Unicode normalization are refused at plan time", async () =>
{
...
test("cell 12: a member with no read permission refuses the bundle before any byte is copied",
    { skip: process.getuid?.() === 0 && "root reads a mode-000 file, so this machine cannot express the case" }, async () =>
    {
...
test("cell 13: 1001 files refuse the bundle, naming the count and the bound, and leave the store untouched", async () =>
{
    const before = storeTree();
    const { code, out } = await attach(["--artifact", manyFiles("wide", 1001)]);
    assert.equal(code, 1, out);
...
test("cell 14: 1000 files over 100 MiB refuse the bundle, naming the total and the bound", async () =>
{
    const before = storeTree();
    const { code, out } = await attach(["--artifact", manyFiles("heavy", 1000, 110 * 1024)]);
    assert.equal(code, 1, out);
    assert.match(out, new RegExp(`holds ${1000 * 110 * 1024} bytes, over the ${100 * 1024 * 1024}-byte bound`));
    assert.deepEqual(storeTree(), before);
});

test("cell 15: exactly 1000 files under 100 MiB ingest", async () =>
{
...
test("cell 16: index.html wins over README.md at the root", async () =>
{
    assert.equal((await entryOf("site", { "index.html": "a", "README.md": "b" })).entry, "index.html");
});

test("cell 17: index.md wins over README.md at the root", async () =>
{
    assert.equal((await entryOf("site", { "index.md": "a", "README.md": "b" })).entry, "index.md");
});

test("cell 18: README.md alone is the entry", async () =>
{
    assert.equal((await entryOf("site", { "README.md": "b" })).entry, "README.md");
});

test("cell 19: an index.html in a subdirectory is not adopted; a root index.html is generated", async () =>
{
    const meta = await entryOf("site", { "docs/index.html": "a", "notes.txt": "n" });
    assert.equal(meta.entry, "index.html");
    assert.equal(meta.members.find((member) => member.path === "index.html").generated, true);
});

test("cell 20: a directory named index.html is not a candidate; README.md is the entry", async () =>
{
    const meta = await entryOf("site", { "index.html/part.txt": "p", "README.md": "b" });
    assert.equal(meta.entry, "README.md");
    assert.deepEqual(memberPaths(meta), ["README.md", "index.html/part.txt"]);
});

test("cell 21: with no candidate a root index.html is generated, linking every brought member and not itself", async () =>
{
    ...
    assert.match((await must(box, demo, ["artifact", "list"])).out, /site\/ \(7 files\)/);
});

/* ── 22–27: --entry ────────────────────────────────────────────────── */

test("cell 22: --entry names a member, and nothing is generated", async () =>
{
    const meta = await entryOf("site", { "docs/main.html": "m", "notes.txt": "n" }, ["--entry", "docs/main.html"]);
...
test("cell 23: --entry naming no member is refused", async () =>
{
    const { code, out } = await attach(["--artifact", tree("site", { "a.txt": "a" }), "--entry", "nope.html"]);
    assert.equal(code, 1, out);
    assert.match(out, /--entry nope\.html is not a member of "site"/);
});

test("cell 24: --entry outside the bundle's root is refused, absolute or relative", async () =>
{
    for (const entry of ["../x", "/abs/x"])
    {
        const { code, out } = await attach(["--artifact", tree("site", { "a.txt": "a" }), "--entry", entry]);
        assert.equal(code, 1, `${entry}: ${out}`);
        assert.match(out, /is not a member path/);
    }
});

test("cell 25: --entry naming a directory is refused", async () =>
{
    const { code, out } = await attach(["--artifact", tree("site", { "docs/a.txt": "a" }), "--entry", "docs"]);
    assert.equal(code, 1, out);
    assert.match(out, /--entry docs names a directory inside "site"/);
});

test("cell 26: --entry beside a single-file --artifact is refused", async () =>
{
    const { code, out } = await attach(["--artifact", fileAt("notes.md", "n"), "--entry", "x"]);
    assert.equal(code, 1, out);
    assert.match(out, /this report attaches no directory/);
});

test("cell 27: --entry beside two directory --artifacts is refused", async () =>
{
    const { code, out } = await attach([
...
test("cell 28: a bundle and a file in one report are two artifact records and two rows", async () =>
{
    const { work, code, out } = await attach([
    ...
    const rows = (await must(box, demo, ["artifact", "list", "--work", work])).out.split("\n").filter((line) => line.includes("a-"));
    assert.equal(rows.length, 2);
    assert.ok(rows.some((row) => row.endsWith("dist/ (2 files)")));
    assert.ok(rows.some((row) => row.endsWith(metas[1].name)));
});

test("cell 29: one path declared twice in a report is refused", async () =>
{
    const root = tree("dist", { "index.html": "i" });
    const { code, out } = await attach(["--artifact", root, "--artifact", root]);
    assert.equal(code, 1, out);
    assert.match(out, /is declared twice in this report/);
});

test("cell 30: a path inside a declared bundle is refused", async () =>
{
    const root = tree("dist", { "index.html": "i" });
    const { code, out } = await attach(["--artifact", root, "--artifact", join(root, "index.html")]);
    assert.equal(code, 1, out);
...
test("cell 31: a design report carries a bundle as its one artifact, and the receipt names report confirm", async () =>
{
    const decision = idIn((await must(box, demo, ["decide", "cell 31: bundles are one artifact"])).out);
    const work = await unit();
    const result = await selfIn(box, demo, ["report", work, "the design", "--design", "--implements", decision,
...
{
    const decision = idIn((await must(box, demo, ["decide", "cell 32: an approval binds bytes"])).out);
    const work = await unit();
    const result = await must(box, demo, ["report", work, "the design", "--design", "--implements", decision,
...
test("cell 33: a copy failing on the third member leaves the store exactly as it was found", async () =>
{
...
test("cell 34: artifact list states a bundle as one row with its file count, and counts artifacts", async () =>
{
    const listed = (await must(reading.box, reading.ws, ["artifact", "list"])).out;
...
test("cell 35: a search matching only a member shows the bundle's one row", async () =>
{
    const found = (await must(reading.box, reading.ws, ["artifact", "search", "logo.svg"])).out;
...
test("cell 36: a search matching the bundle's name shows the same one row", async () =>
{
    const found = (await must(reading.box, reading.ws, ["artifact", "search", "dist"])).out;
    assert.match(found, /dist\/ \(12 files\)/);
    assert.match(found, /^1 artifact$/m);
});
...
test("cell 37: artifact open at a terminal opens the entry, and the receipt names it with the id", async () =>
{
    ...
    const { work } = await attach(["--artifact", root]);
    const meta = attached(work)[0];
...
{
    const { work } = await attach(["--artifact", tree("dist", { "index.html": "hi", "a.txt": "a" })]);
    const meta = attached(work)[0];
    const printed = (await must(box, demo, ["artifact", "open", meta.id], { SUPERSELF_SESSION: "s" })).out;
...
test("cell 39: a bundle whose entry is not in this store is refused, naming the bundle and self sync", async () =>
{
    const { work } = await attach(["--artifact", tree("dist", { "index.html": "hi" })]);
    const meta = attached(work)[0];
    chmodSync(join(store, meta.path), 0o700);
    rmSync(join(store, meta.path, "index.html"), { force: true });
    const { code, out } = await selfIn(box, demo, ["artifact", "open", meta.id]);
    assert.equal(code, 1, out);
...
test("cell 40: work show and the folded work document both state the bundle as one row with no digest", async () =>
{
    const shown = (await must(reading.box, reading.ws, ["work", "show", readingBundle.work])).out;
...
test("cell 41: self search on a member name resolves to the work unit that carries it", async () =>
{
    const found = (await must(reading.box, reading.demo, ["search", "logo.svg"])).out;
    assert.ok(found.includes(readingBundle.work), `a member name did not reach its work unit:\n${found}`);
    assert.match(found, /^1 match$/m);
});

test("cell 42: the HTML views link a bundle to its entry, with a folder plate and the file count", async () =>
{
    ...
    const { work } = await attach(["--artifact", tree("marked", { "a#b.html": "h", "n.txt": "n" }), "--entry", "a#b.html"]);
...
test("cell 43: a member edited in the store raises one signal naming the artifact id and that member's path", async () =>
{
    const { work } = await attach(["--artifact", tree("dist", { "index.html": "hi", "assets/logo.svg": "<svg/>" })]);
    const meta = attached(work)[0];
...
test("cell 44: a member deleted from the store raises one missing signal pointing at self sync", async () =>
{
    const { work } = await attach(["--artifact", tree("dist", { "index.html": "hi", "assets/gone.svg": "<svg/>" })]);
    const meta = attached(work)[0];
...
test("cell 45: a bundle event read without its manifest lists as one directory row, opens the directory, and raises no signal", async () =>
{
    const place = await freshMachine();
    ...
    await must(place.box, place.demo, ["report", work, "attached", "--artifact", root, "--friction", "예상대로"]);
    ...
    const listed = (await must(place.box, place.ws, ["artifact", "list"])).out;
    assert.match(listed, /  dist$/m, "an unread manifest still printed a file count");
    assert.ok(!listed.includes("files)"));
    const opened = (await must(place.box, place.demo, ["artifact", "open", meta.id], { SUPERSELF_SESSION: "s" })).out;
    assert.equal(opened.split(" — ")[0], join(realpathSync(join(place.ws, ".superself")), meta.path));
    assert.match((await must(place.box, place.demo, ["status"])).out, /health: ok/);
});
...
test("cell 52: a design report whose manifest is not read refuses confirm, and the newer CLI still confirms it", async () =>
{
    ...
    const refused = await selfIn(place.box, place.demo, ["report", "confirm", report]);
...
test("cell 46: an existing single-file artifact folds, lists, searches, opens and shows exactly as before", async () =>
{
    const { work } = await attach(["--artifact", fileAt("result.json", "{\"ok\":true}\n")]);
    const meta = attached(work)[0];
    ...
    const row = (await must(box, demo, ["artifact", "list", "--work", work])).out.split("\n")[0];
    assert.equal(row, `${meta.id}  ${reportFor(work).ts.slice(0, 10)}  demo  ${work}  ${meta.name}`);
    assert.match((await must(box, demo, ["artifact", "search", meta.name])).out, new RegExp(`${meta.id}\\b`));
    const opened = (await must(box, demo, ["artifact", "open", meta.id], { SUPERSELF_SESSION: "s" })).out;
    ...
    assert.ok((await must(box, demo, ["work", "show", work])).out.includes(`${meta.id} ${meta.name} (${meta.digest})`) === false);
    assert.ok((await must(box, demo, ["work", "show", work])).out.includes(`- Artifacts: ${meta.id} ${meta.name}`));
});

/* ── 47–49: what the verbs resolve from outside their arguments ────── */

test("cell 47: --artifact ./dist resolves against the directory the command ran in", async () =>
{
    ...
    const { work, code, out } = await attach(["--artifact", "./dist"], sub);
    assert.equal(code, 0, out);
    const meta = attached(work)[0];
...
test("cell 48: the bytes land under the owning log's slug, not the project the command ran in", async () =>
{
    const place = await freshMachine();
    ...
    mkdirSync(root, { recursive: true });
    writeFileSync(join(root, "index.html"), "hi");
    const result = await selfIn(place.box, other, ["report", work, "attached", "--artifact", root]);
    assert.equal(result.code, 0, result.out);
    const placeLog = join(place.ws, ".superself", "projects", "demo", "log.jsonl");
...
test("cell 49: the empty listing's advertised command is unchanged, and is refused outside a project", async () =>
{
    const place = await freshMachine();
...
test("cell 50: work done accepts a unit whose only evidence is a bundle", async () =>
{
    const place = await freshMachine();
    const root = join(place.box.root, "dist");
    mkdirSync(root, { recursive: true });
...
test("cell 53: 1000 brought files with no root candidate ingest, and the generated index is the 1001st", async () =>
{
    const { work, code, out } = await attach(["--artifact", manyFiles("brim", 1000)]);
    assert.equal(code, 0, out);
    const meta = attached(work)[0];
    ...
    assert.match((await must(box, demo, ["artifact", "list", "--work", work])).out, /brim\/ \(1001 files\)/);
});

test("cell 54: a root directory named index.html with no other candidate is refused, naming --entry", async () =>
{
    const entries = { "index.html/part.txt": "p", "notes.txt": "n" };
    const before = storeTree();
    const { code, out } = await attach(["--artifact", tree("site", entries)]);
    assert.equal(code, 1, out);
...
test("cell 56: an entry climbing out of the store refuses artifact open, and launches nothing", async () =>
{
    const place = await freshMachine();
    const { meta } = await hostileBundle(place, (artifact) => { artifact.entry = "../../../../../../etc/passwd"; });
    const refused = await selfIn(place.box, place.demo, ["artifact", "open", meta.id], { SUPERSELF_SESSION: "s" });
    assert.equal(refused.code, 1, refused.out);
...
test("cell 57: one directory reached by two spellings is one path, and containment reads the followed path", async () =>
{
    const root = tree("dist", { "index.html": "i" });
    const link = join(dirname(root), "link-to-dist");
    symlinkSync(root, link);
    const twice = await attach(["--artifact", root, "--artifact", link]);
    assert.equal(twice.code, 1, twice.out);
    assert.match(twice.out, /is declared twice in this report/);
    const inside = await attach(["--artifact", link, "--artifact", join(root, "index.html")]);
    assert.equal(inside.code, 1, inside.out);
    assert.match(inside.out, /which this report attaches as a bundle/);
});

test("cell 58: --entry passed twice is refused by name, not silently narrowed to the last", async () =>
{
    const root = tree("site", { "a.html": "a", "b.html": "b" });
    const { work, code, out } = await attach(["--artifact", root, "--entry", "a.html", "--entry", "b.html"]);
    assert.equal(code, 1, out);
    assert.match(out, /--entry names one member and was passed 2 times/);
    assert.equal(reportFor(work), undefined, "a refused report reached the log");
    // One --entry is unchanged.
    assert.equal((await entryOf("site", { "a.html": "a", "b.html": "b" }, ["--entry", "b.html"])).entry, "b.html");
});
...
// broken checkout.
test("cell 60: a root name that folds to index.html refuses generation at plan time, naming --entry", async () =>
{
    ...
    {
        const before = storeTree();
        const { work, code, out } = await attach(["--artifact", tree("site", entries)]);
    ...
    const named = await entryOf("site", { "INDEX.HTML": "u", "notes.txt": "n" }, ["--entry", "notes.txt"]);
    assert.equal(named.entry, "notes.txt");
    ...
    const beside = await entryOf("site", { "README.MD": "r", "INDEX.MD": "i" });

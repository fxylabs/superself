...
must(box, demo, ["project", "init", "--name", "demo", "--desc", "wrong-kind hints", "--no-connect"]);

test("`work show` with an objective id names the objective surface", () =>
{
    const answer = selfIn(box, demo, ["work", "show", "o-gn3t3"]);
...
test("`work show` with a milestone id names the milestone surface", () =>
{
    const answer = selfIn(box, demo, ["work", "show", "m-abcde"]);
    assert.notEqual(answer.code, 0);
    assert.match(answer.out, /"m-abcde" is a milestone id, not a work id — run `self milestone show m-abcde`/);
});

test("`objective show` with a work id names the work surface", () =>
{
    const answer = selfIn(box, demo, ["objective", "show", "w-abcde"]);
    assert.notEqual(answer.code, 0);
    assert.match(answer.out, /"w-abcde" is a work id, not an objective id — run `self work show w-abcde`/);
});

test("`milestone show` with a work id names the work surface", () =>
{
    const answer = selfIn(box, demo, ["milestone", "show", "w-abcde"]);
    assert.notEqual(answer.code, 0);
    assert.match(answer.out, /"w-abcde" is a work id, not a milestone id — run `self work show w-abcde`/);
});

test("`work show` with a missing work id keeps the unknown-id refusal", () =>
{
    const answer = selfIn(box, demo, ["work", "show", "w-zzzzz"]);
    assert.notEqual(answer.code, 0);

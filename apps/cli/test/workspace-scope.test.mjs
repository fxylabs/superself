...
{
    return (await must(box, other, ["context"])).out;
}
...
test("D1: state add --scope workspace renders in every project's context, from the home store", async () =>
{
    shared = entityIn((await must(box, demo, ["state", "add", "one shared truth", "--scope", "workspace", "--priority", "5"])).out);
    assert.ok((await must(box, demo, ["context"])).out.includes("one shared truth"), "the home project lost its own workspace entity");
    assert.ok((await otherContext()).includes("one shared truth"), "the workspace entity did not reach the other project's context");
...
test("D2: state place --scope workspace moves an entity into every context by placement event", async () =>
{
    const local = entityIn((await must(box, demo, ["state", "add", "promoted note", "--priority", "6"])).out);
    assert.ok(!(await otherContext()).includes("promoted note"));
    await must(box, demo, ["state", "place", local, "--scope", "workspace"]);
    assert.ok((await must(box, demo, ["state", "show", local])).out.includes("placement: workspace · index · priority 6"));
    assert.ok((await otherContext()).includes("promoted note"), "a workspace placement did not reach the other context");
    // D3: back to one project's context only. The `project` keyword was retired
    // by #181, so the home project is named by its slug.
    await must(box, demo, ["state", "place", local, "--scope", "demo"]);
    assert.ok(!(await otherContext()).includes("promoted note"), "a project placement still renders workspace-wide");
    assert.ok((await must(box, demo, ["context"])).out.includes("promoted note"));
});

test("D4: the caps count per scope value, so the tiers fill and gate independently", async () =>
{
    setCaps({ indexTokens: 14 }, capWs);
    await must(capBox, capDemo, ["state", "add", "project seat"]);
    const projectRefused = await capSelf(["state", "add", "one over"]);
    ...
    capShared = entityIn((await must(capBox, capDemo, ["state", "add", "workspace seat", "--scope", "workspace"])).out);
    const workspaceRefused = await capSelf(["state", "add", "workspace over", "--scope", "workspace"]);
    assert.notEqual(workspaceRefused.code, 0);
    assert.match(workspaceRefused.out, /the workspace index tier holds 14 of 14 tokens/);
});

test("D5: a capped workspace tier refuses with the same one-pass shape, and --demote frees it", async () =>
{
    ...
    const refused = await capSelf(["state", "add", "workspace over", "--scope", "workspace"]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /--demote <id>/);
    assert.match(refused.out, /self state place <id> --exposure search --why/);
    const projectSeat = (await must(capBox, capDemo, ["state", "list"])).out.match(/\be-[0-9a-z]{5}\b(?=.*project seat)/)[0];
    const wrongScope = await capSelf(["state", "add", "workspace over", "--scope", "workspace", "--demote", projectSeat]);
    ...
    const admitted = entityIn((await must(capBox, capDemo, ["state", "add", "workspace over", "--scope", "workspace", "--demote", capShared])).out);
    assert.ok((await must(capBox, capDemo, ["state", "show", capShared])).out.includes("placement: workspace · search"));
    assert.ok((await must(capBox, capDemo, ["state", "show", admitted])).out.includes("placement: workspace · index"));
});

test("D6: convention add --workspace records at workspace scope and renders everywhere", async () =>
{
    const rule = idIn((await must(box, demo, ["convention", "add", "one shared rule", "--workspace"])).out);
    assert.ok((await must(box, demo, ["state", "show", rule])).out.includes("placement: workspace · full · priority 30"));
    assert.ok((await otherContext()).includes("one shared rule"), "a workspace convention did not reach the other project's context");
});

test("D7: workspace and project entities interleave by priority in one ordering", async () =>
{
    await must(box, other, ["goal", "add", "the other project's goal"]);
    await must(box, other, ["objective", "add", "the other project's objective"]);
    await must(box, demo, ["state", "place", shared, "--exposure", "full", "--why", "promoted for the ordering check"]);
    const context = await otherContext();

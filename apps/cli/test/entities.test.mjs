...
async function appendLegacy(lines)
{
...
async function appendEntity(lines)
{
    const log = join(box.root, "ws", ".superself", "projects", "demo", "log.jsonl");
    appendFileSync(log, lines.map((line) => JSON.stringify(line) + "\n").join(""));
    await must(box, demo, ["fold"]);
...
test("a legacy goal.set chain folds to exactly one live goal entity with supersede lineage", async () =>
{
    ...
    const list = (await must(box, demo, ["state", "list"])).out;
    ...
    const replaced = (await must(box, demo, ["state", "show", first])).out;
    assert.ok(replaced.includes("superseded"));
    assert.ok(replaced.includes(`superseded by: ${second}`));
    assert.ok((await must(box, demo, ["state", "show", second])).out.includes(`link: supersedes ${first}`));
});

test("decision lifecycles read as decision-labeled index entities", async () =>
{
    const declined = idIn((await must(box, demo, ["decide", "try direction a", "--proposed"])).out);
    await must(box, demo, ["decide", "decline", declined, "--why", "not now"]);
    const retracted = idIn((await must(box, demo, ["decide", "hold direction b"])).out);
    await approved(["decide", "retract", retracted, "--why", "walked back"], retracted);
    const replaced = idIn((await must(box, demo, ["decide", "ship weekly"])).out);
    const successor = idIn((await approved(["decide", "ship daily", "--supersedes", replaced], replaced)).printed);
    const live = (await must(box, demo, ["state", "show", successor])).out;
    ...
    assert.ok((await must(box, demo, ["state", "show", replaced])).out.includes(`superseded by: ${successor}`));
    const down = (await must(box, demo, ["state", "show", declined])).out;
    ...
    assert.ok((await must(box, demo, ["state", "show", retracted])).out.includes("closed: walked back"));
    const list = (await must(box, demo, ["state", "list"])).out;
...
{
    const dropped = idIn((await must(box, demo, ["convention", "add", "tabs everywhere"])).out);
    await approved(["convention", "drop", dropped, "--why", "spaces won"], dropped);
    const shown = (await must(box, demo, ["state", "show", dropped])).out;
    assert.ok(shown.includes("retracted"));
    assert.ok(shown.includes("closed: spaces won"));
    assert.ok(shown.includes("labels: convention"));
    const old = idIn((await must(box, demo, ["convention", "add", "four space indent"])).out);
    const corrected = idIn((await approved(["convention", "add", "four space indent, semicolons", "--supersedes", old], old)).printed);
    assert.ok((await must(box, demo, ["state", "show", old])).out.includes(`superseded by: ${corrected}`));
    assert.ok((await must(box, demo, ["state", "show", corrected])).out.includes("placement: project · full · priority 30"));
});

test("legacy objective create, revise and close fold to one full-exposure objective entity", async () =>
{
    await appendLegacy([
    ...
    let shown = (await must(box, demo, ["state", "show", "o-lega1"])).out;
    ...
    assert.ok((await must(box, demo, ["state", "show", "o-lega1"])).out.includes("target: 2030-06-30"),
    ...
    shown = (await must(box, demo, ["state", "show", "o-lega1"])).out;
    assert.ok(shown.includes("retracted"));
    assert.ok(shown.includes("closed: descoped"));
    assert.ok(!(await must(box, demo, ["state", "list"])).out.includes("reach preview"));
});

test("a reached legacy objective and a declined legacy proposal both leave the live entity set", async () =>
{
    await appendLegacy([
    ...
    const closed = (await must(box, demo, ["state", "show", "o-lega2"])).out;
    ...
    assert.ok((await must(box, demo, ["state", "list"])).out.includes("maybe someday"), "a proposed objective is a live entity");
    await appendLegacy([
    ...
    assert.ok(!(await must(box, demo, ["state", "list"])).out.includes("maybe someday"));
    assert.ok((await must(box, demo, ["state", "show", "o-lega3"])).out.includes("closed: off goal"));
});

test("a legacy milestone folds to an index entity with criteria and its objective grouping", async () =>
{
    await appendLegacy([
    ...
    assert.ok((await must(box, demo, ["state", "show", "m-lega1"])).out.includes("superseded by: m-lega2"));
    assert.ok((await must(box, demo, ["state", "show", "m-lega2"])).out.includes("link: supersedes m-lega1"));
    await appendLegacy([
    ...
    const droppedShown = (await must(box, demo, ["state", "show", "m-lega2"])).out;
    assert.ok(droppedShown.includes("retracted"));
    assert.ok(droppedShown.includes("closed: checkpoint removed"));
});

test("an orphan milestone folds to nothing, and merge-ordered entity lines still settle", async () =>
{
    const log = join(box.root, "ws", ".superself", "projects", "demo", "log.jsonl");
    const lines = [
    ...
    const merged = (await must(box, demo, ["state", "show", "e-mrgd1"])).out;
    assert.ok(merged.includes("retracted"));
    assert.ok(merged.includes("closed: withdrawn elsewhere"));
    assert.ok((await must(box, demo, ["state", "show", "e-mrgd2"])).out.includes("superseded by: e-mrgd3"));
    assert.ok(list.includes("newer statement"));
});

test("a standalone supersession claim settles only when its successor exists and was confirmed", async () =>
{
    const log = join(box.root, "ws", ".superself", "projects", "demo", "log.jsonl");
    const lines = [
    ...
    assert.ok((await must(box, demo, ["state", "show", "e-supa1"])).out.includes("confirmed"));
});

test("the creation is v1 however late its timestamp reads, and the rest sort by (ts, event id)", async () =>
{
    const id = "w-revn1";
    await appendEntity([
...
test("an acceptance naming a revision event confirms the record it belongs to", async () =>
{
    const id = "w-revn2";
    await appendEntity([
    ...
    await appendEntity([revisionLine(id, "01hz00000000000000000rev13", "2025-06-04T00:00:00.000Z", "the third plan")]);

...
function addGoal(dir, text)
{
    return idIn(must(box, dir, ["goal", "add", text]).out);
}
...
{
    must(box, join(ws, slug), ["context"]);
    return readFileSync(join(ws, ".superself", "projects", slug, "state.md"), "utf8");
}

/* ── A: goal add displaces nothing ─────────────────────────────────── */

test("A1: the first goal records with no gate in the way", () =>
{
    const dir = project("a1");
    const added = selfIn(box, dir, ["goal", "add", "A1 the first direction"]);
    assert.equal(added.code, 0, added.out);
    assert.match(added.out, /entity\.confirmed recorded/);
    assert.match(must(box, dir, ["context"]).out, /A1 the first direction/);
});

test("A2: a second goal stands beside the first, which is left untouched", () =>
{
    const dir = project("a2");
    const first = addGoal(dir, "A2 the first direction");
    const added = selfIn(box, dir, ["goal", "add", "A2 the second direction"]);
    assert.equal(added.code, 0, added.out);
    assert.match(must(box, dir, ["state", "show", first]).out, /confirmed/);
    const context = must(box, dir, ["context"]).out;
    assert.match(context, /A2 the first direction/);
    assert.match(context, /A2 the second direction/);
});

test("A3: a third goal is recorded and all three stand", () =>
{
    const dir = project("a3");
    ["A3 one", "A3 two"].forEach((text) => addGoal(dir, text));
    assert.equal(selfIn(box, dir, ["goal", "add", "A3 three"]).code, 0);
    const context = must(box, dir, ["context"]).out;
...
{
    const dir = project("b1");
    const first = addGoal(dir, "B1 the first direction");
    ...
    assert.match(must(box, dir, ["state", "show", first]).out, /superseded/);
});

test("B2: a supersession from a process with no terminal refuses and records nothing", () =>
{
    const dir = project("b2");
    const first = addGoal(dir, "B2 the standing direction");
    const before = logLines("b2").length;
    const refused = selfIn(box, dir, ["goal", "add", "B2 a replacement", "--supersedes", first]);
...
{
    const dir = project("b3");
    const first = addGoal(dir, "B3 the standing direction");
...
{
    const dir = project("b4");
    const first = addGoal(dir, "B4 one");
    const second = addGoal(dir, "B4 two");
    const third = addGoal(dir, "B4 three");
    const replacing = await approvedIn(box, dir,
        ["goal", "add", "B4 the merged direction", "--supersedes", first, "--supersedes", second], `${first} ${second}`);
    assert.equal(replacing.code, 0, replacing.out);
    assert.match(must(box, dir, ["state", "show", first]).out, /superseded/);
    assert.match(must(box, dir, ["state", "show", second]).out, /superseded/);
    assert.match(must(box, dir, ["state", "show", third]).out, /confirmed/);
    const context = must(box, dir, ["context"]).out;
    assert.match(context, /B4 three/);
    assert.match(context, /B4 the merged direction/);
});

test("B5: --supersedes naming another record kind refuses with that kind's add verb", () =>
{
    const dir = project("b5");
    const decision = idIn(must(box, dir, ["decide", "B5 a decision, not a goal"]).out);
    const refused = selfIn(box, dir, ["goal", "add", "B5 a goal", "--supersedes", decision]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, /self decide/);
});

test("B6: --supersedes naming no record at all refuses", () =>
{
    const dir = project("b6");
    const refused = selfIn(box, dir, ["goal", "add", "B6 a goal", "--supersedes", "01zzzzzzzzzzzzzzzzzzzzzzzz"]);
    assert.equal(refused.code, 1);
...
{
    const dir = project("b7");
    const first = addGoal(dir, "B7 the first direction");
    assert.equal((await approvedIn(box, dir, ["goal", "add", "B7 the second direction", "--supersedes", first], first)).code, 0);
    const again = selfIn(box, dir, ["goal", "add", "B7 a third direction", "--supersedes", first]);
    assert.equal(again.code, 0, again.out);
    assert.match(must(box, dir, ["context"]).out, /B7 a third direction/);
});

test("B8: undo gives a superseded goal back and leaves the successor standing", async () =>
{
    const dir = project("b8");
    const first = addGoal(dir, "B8 the first direction");
    const replacing = await approvedIn(box, dir, ["goal", "add", "B8 the second direction", "--supersedes", first], first);
    const undone = selfIn(box, dir, ["undo", idIn(replacing.printed), "--why", "it stands beside the first, it does not replace it"]);
...
test("B9: goal add takes no --why, and says what --why is for", () =>
{
    const dir = project("b9");
    const refused = selfIn(box, dir, ["goal", "add", "B9 a goal", "--why", "a reason"]);
    assert.equal(refused.code, 1);
...
test("C1: goal set refuses, names goal add, and records nothing", () =>
{
    const dir = project("c1");
    const before = logLines("c1").length;
    const refused = selfIn(box, dir, ["goal", "set", "C1 a direction"]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, /goal add/);
    assert.equal(logLines("c1").length, before);
    assert.doesNotMatch(must(box, dir, ["context"]).out, /C1 a direction/);
});

test("C2: goal set refuses with a goal standing, and the standing goal is untouched", () =>
{
    const dir = project("c2");
    const first = addGoal(dir, "C2 the standing direction");
    assert.equal(selfIn(box, dir, ["goal", "set", "C2 a replacement"]).code, 1);
    assert.match(must(box, dir, ["state", "show", first]).out, /confirmed/);
});
...
{
    const dir = project("d1");
    const first = addGoal(dir, "D1 the abandoned direction");
    addGoal(dir, "D1 the kept direction");
...
{
    const dir = project("d2");
    const only = addGoal(dir, "D2 the only direction");
    assert.equal((await approvedIn(box, dir, ["goal", "retract", only, "--why", "it was never this project's"], only)).code, 0);
    assert.doesNotMatch(must(box, dir, ["context"]).out, /D2 the only direction/);
    assert.match(must(box, dir, ["status"]).out, /not set/);
});

test("D3: a retraction from a process with no terminal refuses and records nothing", () =>
{
    const dir = project("d3");
    const only = addGoal(dir, "D3 the standing direction");
    const before = logLines("d3").length;
    const refused = selfIn(box, dir, ["goal", "retract", only, "--why", "no longer holds"]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, /nothing was recorded/);
    assert.equal(logLines("d3").length, before);
});

test("D4: goal retract without --why is refused before anything is disclosed", () =>
{
    const dir = project("d4");
    const only = addGoal(dir, "D4 the standing direction");
    const refused = selfIn(box, dir, ["goal", "retract", only]);
    assert.equal(refused.code, 1);
...
{
    const dir = project("d5");
    const only = addGoal(dir, "D5 the standing direction");
    assert.equal((await approvedIn(box, dir, ["goal", "retract", only, "--why", "it no longer holds"], only)).code, 0);
    const again = selfIn(box, dir, ["goal", "retract", only, "--why", "again"]);
    assert.equal(again.code, 1);
    assert.match(again.out, /already retracted/);
});

test("D6: goal retract on another record kind refuses", () =>
{
    const dir = project("d6");
    const decision = idIn(must(box, dir, ["decide", "D6 a decision, not a goal"]).out);
    const refused = selfIn(box, dir, ["goal", "retract", decision, "--why", "wrong verb"]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, /is not a goal/);
});

test("D7: goal retract takes no --supersedes, and says which verb replaces a goal", () =>
{
    const dir = project("d7");
    const only = addGoal(dir, "D7 the standing direction");
    const refused = selfIn(box, dir, ["goal", "retract", only, "--why", "w", "--supersedes", only]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, /goal add/);
});
...
test("E1: bare goal refuses with both verbs in the usage line", () =>
{
    const refused = selfIn(box, demo, ["goal"]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, /goal add/);
...
test("F1: with no goal, the file says not set and the one-line surfaces agree", () =>
{
    const dir = project("f1");
    assert.match(stateFile("f1"), /## Goal\n\n_not set_/);
    assert.match(must(box, dir, ["status"]).out, /not set/);
});

test("F2: one goal renders as itself, with nothing counted behind it", () =>
{
    const dir = project("f2");
    addGoal(dir, "F2 the only direction");
    assert.match(stateFile("f2"), /## Goal\n\nF2 the only direction/);
    const status = must(box, dir, ["status"]).out;
    assert.match(status, /F2 the only direction/);
    assert.doesNotMatch(status, /\+\d+ more/);
});

test("F3: three goals all render in the file, and the one-line surfaces count the rest", () =>
{
    const dir = project("f3");
    ["F3 one", "F3 two", "F3 three"].forEach((text) => addGoal(dir, text));
    const file = stateFile("f3");
    ["F3 one", "F3 two", "F3 three"].forEach((text) => assert.match(file, new RegExp(text)));
    assert.match(must(box, dir, ["status"]).out, /\(\+2 more\)/);
    const context = must(box, dir, ["context"]).out;

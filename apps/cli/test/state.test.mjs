...
test("state add records an entity with placement, metadata and rationale", async () =>
{
    const id = entityIn((await must(box, demo, ["state", "add", "weekly retro every friday",
...
test("a proposed entity waits for confirm, and confirming twice is named", async () =>
{
    const id = entityIn((await must(box, demo, ["state", "add", "adopt trunk based flow", "--proposed"])).out);
    assert.ok((await must(box, demo, ["state", "list"])).out.includes("(proposed)"));
    await must(box, demo, ["state", "confirm", id]);
    assert.ok((await must(box, demo, ["state", "show", id])).out.includes("confirmed"));
...
{
    const id = entityIn((await must(box, demo, ["state", "add", "temporary rule"])).out);
    const bare = await self(["state", "retract", id]);
    assert.notEqual(bare.code, 0);
    assert.match(bare.out, /--why/);
    await approved(["state", "retract", id, "--why", "no longer needed"], id);
    assert.ok(!(await must(box, demo, ["state", "list"])).out.includes("temporary rule"));
    const shown = (await must(box, demo, ["state", "show", id])).out;
    assert.ok(shown.includes("retracted"));
    assert.ok(shown.includes("closed: no longer needed"));
    const again = await self(["state", "retract", id, "--why", "twice"]);
    assert.notEqual(again.code, 0);
    ...
    assert.equal((await must(box, demo, ["search", "temporary rule"])).out.trim(), "no matches");
    assert.match((await must(box, demo, ["state", "show", id, "--history"])).out, /retracted/);
});

test("a confirmed successor supersedes; a proposal displaces nothing until confirmed", async () =>
{
    const first = entityIn((await must(box, demo, ["state", "add", "release on mondays"])).out);
    const second = entityIn((await must(box, demo, ["state", "add", "release on demand",
        "--link", `supersedes:${first}`, "--proposed"])).out);
    assert.ok((await must(box, demo, ["state", "show", first])).out.includes("confirmed"),
        "a proposal displaced a confirmed entity");
    await must(box, demo, ["state", "confirm", second]);
    assert.ok((await must(box, demo, ["state", "show", first])).out.includes(`superseded by: ${second}`));
    const third = await self(["state", "add", "release hourly", "--link", `supersedes:${first}`]);
    assert.notEqual(third.code, 0);
    assert.match(third.out, /already superseded/);
});

test("links resolve, type-check, and refuse duplicates and unknown targets", async () =>
{
    const anchor = entityIn((await must(box, demo, ["state", "add", "platform theme"])).out);
    const grouped = entityIn((await must(box, demo, ["state", "add", "billing rework",
        "--link", `member-of:${anchor}`, "--link", anchor])).out);
    const shown = (await must(box, demo, ["state", "show", grouped])).out;
    assert.ok(shown.includes(`link: member-of ${anchor}`));
    assert.ok(shown.includes(`link: relates ${anchor}`));
    const badType = await self(["state", "add", "x", "--link", `blocks:${anchor}`]);
    assert.notEqual(badType.code, 0);
    assert.match(badType.out, /member-of:<id>, supersedes:<id>, or relates:<id>/);
    const unknown = await self(["state", "add", "x", "--link", "e-zzzzz"]);
    assert.notEqual(unknown.code, 0);
    assert.match(unknown.out, /unknown entity "e-zzzzz"/);
    const doubled = await self(["state", "add", "x", "--link", anchor, "--link", anchor]);
    assert.notEqual(doubled.code, 0);
    assert.match(doubled.out, /repeated/);
});

test("placement and metadata shapes are refused in user terms", async () =>
{
    const refusals = [
    ...
    {
        const result = await self(args);
        assert.notEqual(result.code, 0, `\`${args.join(" ")}\` was accepted`);
        assert.match(result.out, message, `\`${args.join(" ")}\``);
    }
});

test("the preset record kinds keep their own lifecycle verbs", async () =>
{
    const goal = idIn((await must(box, demo, ["goal", "add", "own the niche"])).out);
    const retractGoal = await self(["state", "retract", goal, "--why", "x"]);
    assert.notEqual(retractGoal.code, 0);
    assert.match(retractGoal.out, /goal record — run `self goal retract <id> --why w`/);
    const decision = idIn((await must(box, demo, ["decide", "keep sqlite", "--proposed"])).out);
    const confirmLegacy = await self(["state", "confirm", decision]);
    assert.notEqual(confirmLegacy.code, 0);
    assert.match(confirmLegacy.out, /decide confirm/);
    const supersedeLegacy = await self(["state", "add", "keep postgres", "--link", `supersedes:${decision}`]);
    ...
    const retractLegacy = await self(["state", "retract", convention, "--why", "x"]);
    assert.notEqual(retractLegacy.code, 0);
    assert.match(retractLegacy.out, /convention drop/);
});

test("state writes declare no scope flag, and reads answer for a named project", async () =>
{
    const scoped = await self(["state", "add", "x", "--project", "demo"]);
    assert.notEqual(scoped.code, 0);
    assert.match(scoped.out, /unknown option '--project'/);
    assert.ok((await must(box, ws, ["state", "list", "--project", "demo"])).out.includes("weekly retro"),
        "a named-project read did not answer from outside the checkout");
    const extra = await self(["state", "list", "extra"]);
    assert.notEqual(extra.code, 0);
    assert.match(extra.out, /unexpected argument 'extra'/);
    const unknown = await self(["state", "show", "e-zzzzz"]);
    assert.notEqual(unknown.code, 0);

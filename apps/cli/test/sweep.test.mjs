...
function fresh(...slugs)
{
    ...
    for (const slug of slugs)
    {
        ...
        must(box, dirs[slug], ["project", "init", "--name", slug, "--desc", `${slug} project`, "--no-connect"]);
    }
...
function reported(box, dir, said)
{
    seq += 1;
    const unit = workIdIn(must(box, dir, ["work", "add", `outcome ${seq}`]).out);
    must(box, dir, ["report", unit, `report ${seq}`, "--friction", said]);
    return unit;
}
...
function reportedDaysAgo(box, ws, project, dir, said, days)
{
    seq += 1;
    const unit = workIdIn(must(box, dir, ["work", "add", `old outcome ${seq}`]).out);
...
function sweep(box, dir, ...args)
{
    const result = selfIn(box, dir, ["sweep", ...args]);
...
const growing = fresh("demo");

test("B1: an empty window says so, and is not an error", () =>
{
    const out = sweep(growing.box, growing.demo);
    assert.match(out, /no friction was recorded in the last 30 days/);
    assert.match(out, /--friction/);
    assert.equal(clustersIn(out), 0);
});

test("B2: one sentence is below the floor of three, and the run says what it read", () =>
{
    reported(growing.box, growing.demo, SAID);
    const out = sweep(growing.box, growing.demo);
...
test("B3: three sentences that share no word are three sentences, not a cluster", () =>
{
    reported(growing.box, growing.demo, OTHER);
    reported(growing.box, growing.demo, THIRD);
    const out = sweep(growing.box, growing.demo);
...
const clustering = fresh("demo");

test("B4: three near-identical sentences are one cluster showing its three reports", () =>
{
    const evidence = [reported(clustering.box, clustering.demo, SAID),
        NEAR.map((said) => reported(clustering.box, clustering.demo, said))].flat();
    assert.equal(evidence.length, 3);
    const out = sweep(clustering.box, clustering.demo);
...
test("B5: five identical sentences are one cluster with five pieces of evidence", () =>
{
    reported(clustering.box, clustering.demo, SAID);
    reported(clustering.box, clustering.demo, SAID);
    const out = sweep(clustering.box, clustering.demo);
    assert.equal(clustersIn(out), 1);
...
test("B6: a second complaint recorded three times is a second cluster", () =>
{
    for (let index = 0; index < 3; index += 1)
    {
        reported(clustering.box, clustering.demo, OTHER);
    }
    const out = sweep(clustering.box, clustering.demo);
...
test("B7: five 예상대로 sentences cluster into nothing", () =>
{
    const { box, demo } = fresh("demo");
    for (let index = 0; index < 5; index += 1)
    {
        reported(box, demo, index % 2 === 0 ? "예상대로" : "as expected");
    }
    const out = sweep(box, demo);
    assert.match(out, /5 sentences on 5 reports/);
    assert.equal(clustersIn(out), 0);
});

test("B8: the same complaint in two projects is one cluster spanning both", () =>
{
    const { box, dirs } = fresh("demo", "other");
    reported(box, dirs.demo, SAID);
    reported(box, dirs.demo, NEAR[0]);
    reported(box, dirs.other, SAID);
    reported(box, dirs.other, NEAR[1]);
    const out = sweep(box, dirs.demo);
...
const dated = fresh("demo");

test("B9: three sentences older than the window are outside the read", () =>
{
    for (let index = 0; index < 3; index += 1)
    {
        reportedDaysAgo(dated.box, dated.ws, "demo", dated.demo, SAID, 60);
    }
    const out = sweep(dated.box, dated.demo);
    assert.match(out, /no friction was recorded in the last 30 days/);
    assert.equal(clustersIn(out), 0);
});

test("B10: two inside the window and one outside is two, which is below the floor", () =>
{
    reportedDaysAgo(dated.box, dated.ws, "demo", dated.demo, SAID, 5);
    reportedDaysAgo(dated.box, dated.ws, "demo", dated.demo, NEAR[0], 5);
    const out = sweep(dated.box, dated.demo);
    assert.match(out, /2 sentences on 2 reports/);
    assert.equal(clustersIn(out), 0);
});

test("B11: narrowing the window with --since drops a cluster the default finds", () =>
{
    reportedDaysAgo(dated.box, dated.ws, "demo", dated.demo, NEAR[1], 5);
    assert.equal(clustersIn(sweep(dated.box, dated.demo)), 1);
    const narrowed = sweep(dated.box, dated.demo, "--since", "1d");
...
const preview = fresh("demo");

test("B12: a malformed --since is refused, and the refusal shows the form", () =>
{
    reported(preview.box, preview.demo, SAID);
    NEAR.forEach((said) => reported(preview.box, preview.demo, said));
    const result = selfIn(preview.box, preview.demo, ["sweep", "--since", "yesterday"]);
...
test("B13: two previews print the same bytes and record nothing", () =>
{
    const before = eventsOf(preview.ws, "demo").length;
    const first = sweep(preview.box, preview.demo);
    const second = sweep(preview.box, preview.demo);
...
test("B14: a workspace with no registered project says so rather than throwing", () =>
{
    const { box, ws } = fresh();
    const out = sweep(box, ws);
    assert.match(out, /no project is registered in this workspace/);
    assert.match(out, /self project init/);
});

test("B15: friction in an archived project is out of the read", () =>
{
    const { box, ws, dirs } = fresh("demo", "other");
    reported(box, dirs.other, SAID);
    NEAR.forEach((said) => reported(box, dirs.other, said));
    assert.equal(clustersIn(sweep(box, dirs.demo)), 1);
    must(box, ws, ["project", "archive", "other", "--why", "set aside for now"]);
    const out = sweep(box, dirs.demo);
...
const recorded = fresh("demo");
let proposal = null;

test("C1: one cluster records one entity.proposed under a short work id, and its text carries no digit", () =>
{
    reported(recorded.box, recorded.demo, SAID);
    NEAR.forEach((said) => reported(recorded.box, recorded.demo, said));
    const out = sweep(recorded.box, recorded.demo, "--record");
...
test("C2: the proposal reaches a person through the unchanged Waiting on you row", () =>
{
    const out = must(recorded.box, recorded.demo, ["context"]).out;
...
test("C10: the log carries one entity.proposed line for it", () =>
{
    const lines = must(recorded.box, recorded.demo, ["log", "-n", "40"]).out
        .split("\n").filter((line) => line.includes("entity.proposed"));
    assert.equal(lines.length, 1, lines.join("\n"));
    assert.ok(lines[0].includes(SAID), lines[0]);
});

test("C11: search finds it, because it is a live record like any other", () =>
{
    const out = must(recorded.box, recorded.demo, ["search", "recurring friction"]).out;
...
test("C18: an id from refs.friction reaches the report it cites", () =>
{
    const cited = proposalsOf(recorded.ws, "demo")[0].refs.friction[0];
    const line = must(recorded.box, recorded.demo, ["log", "-n", "40"]).out
    ...
    const resolved = selfIn(recorded.box, recorded.demo, ["state", "show", cited]);
    assert.match(resolved.out, new RegExp(`${cited} is a report of w-`), resolved.out);
});

test("C9: recording twice in a row leaves one proposal", () =>
{
    const out = sweep(recorded.box, recorded.demo, "--record");
...
test("C16: one more piece of evidence the next day does not produce a second proposal", () =>
{
    reported(recorded.box, recorded.demo, SAID);
    const out = sweep(recorded.box, recorded.demo, "--record");
...
test("C3: accepting it goes through the unchanged acceptance path", () =>
{
    assert.equal(selfIn(recorded.box, recorded.demo, ["work", "accept", proposal]).code, 0);
    const shown = must(recorded.box, recorded.demo, ["work", "show", proposal]).out;
    assert.ok(shown.includes(SAID), shown);
    assert.ok(!must(recorded.box, recorded.demo, ["context"]).out.includes(`self work accept ${proposal}`));
});
...
const declined = fresh("demo");

test("C4: declining a swept proposal is the unchanged decline", () =>
{
    reported(declined.box, declined.demo, SAID);
    NEAR.forEach((said) => reported(declined.box, declined.demo, said));
    sweep(declined.box, declined.demo, "--record");
    const first = proposalsOf(declined.ws, "demo")[0].payload.entity;
    assert.equal(selfIn(declined.box, declined.demo, ["work", "decline", first, "--why", "not worth systemizing yet"]).code, 0);
    assert.ok(!must(declined.box, declined.demo, ["context"]).out.includes(`self work accept ${first}`));
});

test("C6: the same friction is proposed again after a decline, by design", () =>
{
    const out = sweep(declined.box, declined.demo, "--record");
    assert.match(out, /1 proposal recorded/);
...
test("C5: a cluster whose plan text is already open is skipped, and nothing throws", () =>
{
    const { box, ws, demo } = fresh("demo");
    reported(box, demo, SAID);
    NEAR.forEach((said) => reported(box, demo, said));
    const open = workIdIn(must(box, demo, ["work", "propose", PROPOSED]).out);
    const out = sweep(box, demo, "--record");
    assert.match(out, /0 proposals recorded, 1 cluster skipped/);
...
test("C7: one clashing cluster skips itself while the rest record, at exit 0", () =>
{
    const { box, ws, demo } = fresh("demo");
    reported(box, demo, SAID);
    NEAR.forEach((said) => reported(box, demo, said));
    for (let index = 0; index < 3; index += 1)
    {
        reported(box, demo, OTHER);
    }
    const open = workIdIn(must(box, demo, ["work", "propose", PROPOSED]).out);
    const result = selfIn(box, demo, ["sweep", "--record"]);
    assert.equal(result.code, 0, result.out);
...
test("C8: nothing to propose records nothing, at exit 0", () =>
{
    const { box, ws, demo } = fresh("demo");
    reported(box, demo, SAID);
...
test("C13: --record inside an archived project is refused by the append gate", () =>
{
    const { box, ws, demo } = fresh("demo");
    reported(box, demo, SAID);
    NEAR.forEach((said) => reported(box, demo, said));
    must(box, ws, ["project", "archive", "demo", "--why", "set aside for now"]);
    const result = selfIn(box, demo, ["sweep", "--record"]);
    assert.equal(result.code, 1, result.out);
...
test("C14: evidence spanning projects records into the project the command ran in", () =>
{
    const { box, ws, dirs } = fresh("demo", "other");
    reported(box, dirs.demo, SAID);
    reported(box, dirs.other, NEAR[0]);
    reported(box, dirs.other, NEAR[1]);
    sweep(box, dirs.other, "--record");
    assert.equal(proposalsOf(ws, "demo").length, 0);
...
test("C15: three clusters are one append and one fold", () =>
{
    const { box, ws, demo } = fresh("demo");
    ...
    assert.match(sweep(box, demo, "--record"), /3 proposals recorded/);
...
test("C17: --record in an unregistered directory is refused, naming what to do", () =>
{
    const { box, ws } = fresh("demo");
    const loose = join(ws, "loose");
    mkdirSync(loose, { recursive: true });
    const result = selfIn(box, loose, ["sweep", "--record"]);
    assert.equal(result.code, 1, result.out);

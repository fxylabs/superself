...
function oneLink()
{
    ...
    must(box, a, ["project", "init", "--name", "demo", "--no-connect"]);
...
function folderLink()
{
    ...
    must(box, f, ["project", "init", "--name", "proj", "--no-connect"]);
    const work = workIdIn(must(box, a, ["work", "add", "ship it"]).out);
...
test("E1: a hash on the default branch is settled", () =>
{
    const t = oneLink();
...
test("E2: a hash on another ref is provisional", () =>
{
    const t = oneLink();
...
test("E3: a hash reset away while its branch stands is abandoned, and health says so", () =>
{
    const t = oneLink();
    const hash = branchCommit(t.box, t.a, "f", "two");
    report(t, t.a, hash);
...
test("E4: a dangling hash whose branch is gone is unknown", () =>
{
    const t = oneLink();
    const hash = branchCommit(t.box, t.a, "f", "two");
    report(t, t.a, hash);
...
test("E5: a hash nowhere is unverifiable with today's line", () =>
{
    const t = oneLink();
    report(t, t.a, NOWHERE);
    assert.equal(verdicts(t)[NOWHERE], "unverifiable");
    assert.ok(health(t).includes(`evidence ${NOWHERE} ${GONE1}`), health(t));
});

test("E6: a hash that lives only in a repository that is not linked is unverifiable", () =>
{
    const t = oneLink();
...
test("E7: a stored provisional verdict becomes settled once the branch merges", () =>
{
    const t = oneLink();
    const hash = branchCommit(t.box, t.a, "f", "two");
    report(t, t.a, hash);
...
test("E8 and E9: settled is final — it survives a refold and the object going away", () =>
{
    const t = oneLink();
    const hash = commit(t.box, t.a, "two");
    report(t, t.a, hash);
...
test("E10: a stored unverifiable verdict is walked again once the object is here", () =>
{
    const t = oneLink();
    const hash = commit(t.box, t.a, "two");
...
test("E11, E12, E13: hashes from either repository settle, from whichever checkout the fold runs", () =>
{
    const t = twoLinks();
    const hashA = commit(t.box, t.a, "two");
    const hashB = commit(t.box, t.b, "two");
    report(t, t.a, hashA, hashB);
    ...
    must(t.box, t.ws, ["project", "link", "demo", t.a]);
    assert.deepEqual(verdicts(t), expected);
    assertQuiet(t);
});

test("E14: a hash on another ref of the second repository is provisional", () =>
{
    const t = twoLinks();
...
test("E15: a hash reset away on a branch of the second repository is abandoned", () =>
{
    const t = twoLinks();
    const hash = branchCommit(t.box, t.b, "f", "two");
    report(t, t.b, hash);
...
test("E16: a dangling hash of the second repository whose branch is gone is unknown", () =>
{
    const t = twoLinks();
    const hash = branchCommit(t.box, t.b, "f", "two");
    report(t, t.b, hash);
...
test("E17: a hash no linked repository knows names the repositories asked", () =>
{
    const t = twoLinks();
    report(t, t.a, NOWHERE);
    assert.equal(verdicts(t)[NOWHERE], "unverifiable");
    ...
    assert.ok(must(t.box, t.a, ["status"]).out.includes(goneN("demo, other")));
});
...
test("E18a: a hash two repositories hold is judged in the one the report named", () =>
{
    const t = twoLinks();
    const hash = shared(t);
    report(t, t.b, hash);
...
test("E18b: a legacy report naming no repository is judged in the first linked repository that knows the hash", () =>
{
    const t = twoLinks();
    const hash = shared(t);
...
test("E19: a hash judged unverifiable before its repository was linked settles once it is", () =>
{
    const t = oneLink();
    const hash = commit(t.box, t.b, "two");
    report(t, t.a, hash);
...
test("E20: the evidence head keeps one key per repository, and only the moved one changes", () =>
{
    const t = twoLinks();
    const hash = branchCommit(t.box, t.b, "f", "two");
    report(t, t.a, hash);
...
test("E21: a linked repository missing from this machine demotes nothing", () =>
{
    const t = twoLinks();
    const hash = branchCommit(t.box, t.b, "f", "two");
    report(t, t.a, hash);
...
test("E22: once the dead link is pruned, only the repository left is asked", () =>
{
    const t = twoLinks();
    const hash = branchCommit(t.box, t.b, "f", "two");
    report(t, t.a, hash);
...
test("E23: repositories one level below a linked folder are judged", () =>
{
    const t = folderLink();
    const hashA = commit(t.box, t.a, "two");
    const hashB = commit(t.box, t.b, "two");
...
test("E24: a plain folder and a repository two levels down are not asked", () =>
{
    const t = folderLink();
...
test("E25: a fold run from inside the folder judges the same", () =>
{
    const t = folderLink();
    const hashA = commit(t.box, t.a, "two");
    report(t, t.a, hashA);
...
test("E26: a folder link with no repository below leaves stored verdicts untouched", () =>
{
    const box = machine();
    const ws = join(box.root, "ws");
    mkdirSync(ws, { recursive: true });
    ...
    mkdirSync(f);
    must(box, f, ["project", "init", "--name", "empty", "--no-connect"]);
    const work = workIdIn(must(box, f, ["work", "add", "ship it"]).out);
...
test("E27: no standing link and no registry path leaves stored verdicts untouched", () =>
{
    const t = oneLink();
...
test("E28: a report records the identity of the repository it ran in, beside the branch", () =>
{
    const t = twoLinks();
    report(t, t.a, commit(t.box, t.a, "two"));
    assert.equal(lastReport(t).refs.repository, identity(t.box, t.a));
    assert.equal(lastReport(t).refs.branch, "main");
    report(t, t.b, commit(t.box, t.b, "two"));
    assert.equal(lastReport(t).refs.repository, identity(t.box, t.b));
});

test("E29: a repository with no commit records no repository", () =>
{
    const box = machine();
    const ws = join(box.root, "ws");
    mkdirSync(ws, { recursive: true });
    ...
    mkdirSync(dir);
    git(box, dir, ["init", "-q", "-b", "main"]);
    must(box, dir, ["project", "init", "--name", "fresh", "--no-connect"]);
    const work = workIdIn(must(box, dir, ["work", "add", "ship it"]).out);
    const t = { box, ws, work, store: join(ws, ".superself"), slug: "fresh" };
    report(t, dir, NOWHERE);
    assert.deepEqual(lastReport(t).refs.commits, [NOWHERE]);
    assert.equal(lastReport(t).refs.repository, undefined);
});

test("E31: under a folder project a report reads HEAD, bare hashes, branch and identity from the checkout it ran in", () =>
{
    const t = folderLink();
    const head = commit(t.box, t.a, "two");
    must(t.box, t.a, ["report", t.work, "head by default"]);
    assert.deepEqual(lastReport(t).refs.commits, [head.slice(0, 12)]);
    assert.equal(lastReport(t).refs.branch, "main");
    assert.equal(lastReport(t).refs.repository, identity(t.box, t.a));
    const bare = commit(t.box, t.b, "two");
    must(t.box, t.b, ["report", t.work, "bare hash", "--evidence", bare]);
    assert.deepEqual(lastReport(t).refs.commits, [bare]);
    assert.equal(lastReport(t).refs.repository, identity(t.box, t.b));
    assert.deepEqual(verdicts(t), { [head.slice(0, 12)]: "settled", [bare]: "settled" });
});

test("E30: a fold from the workspace root, outside every repository, settles a second-repository hash", () =>
{
    const t = twoLinks();
    const hash = commit(t.box, t.b, "two");
    report(t, t.a, hash);

...
function twoLinked()
{
    ...
    must(box, ws, ["project", "link", "alpha", b, "--force"]);
    return { box, ws, a, b };
}
...
test("U1: an explicit path is detached, the marker goes with it, and the listing says what is left", () =>
{
    const { box, ws, a, b } = twoLinked();
    const done = must(box, ws, ["project", "unlink", "alpha", b]);
    ...
    assert.match(must(box, ws, ["project", "link", "alpha"]).out, /1 linked path/);
});

test("U2: --here detaches the checkout it runs in, and the listing says this directory is not linked", () =>
{
    const { box, ws, a, b } = twoLinked();
    const done = must(box, b, ["project", "unlink", "alpha", "--here"]);
...
test("U3: run from another checkout of the same project, the listing marks that one as this directory", () =>
{
    const { box, ws, a, b } = twoLinked();
    const done = must(box, a, ["project", "unlink", "alpha", b]);
    assert.ok(done.out.includes(`${a}  (this directory)`), done.out);
    assertPruned(ws, "alpha", b);
});

test("U4: the last standing path is refused without --force, and nothing is written", () =>
{
    const { box, ws, a } = oneLinked();
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha", a]);
...
test("U5: --force detaches the last standing path, says so first, and leaves the project registered", () =>
{
    const { box, ws, a } = oneLinked();
    const done = must(box, ws, ["project", "unlink", "alpha", a, "--force"]);
    ...
    assert.match(must(box, a, ["status"]).out, /alpha/);
    const orphan = selfIn(box, a, ["work", "add", "something"]);
    assert.notEqual(orphan.code, 0);
    assert.ok(orphan.out.includes("not inside a registered project"), orphan.out);
});

test("U6: a path whose checkout is gone is detached, with no --force and no marker to remove", () =>
{
    const { box, ws } = workspace();
    const b = folder(box, ws, "beta");
    must(box, b, ["project", "init", "--name", "alpha", "--no-connect"]);
...
test("U7: a path no project links is refused, naming what the slug does hold", () =>
{
    const { box, ws, a } = oneLinked();
    const c = folder(box, ws, "gamma");
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha", c]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes(`"${c}" is not a linked path of project "alpha", which is linked to ${a}`), refused.out);
    assertUnchanged(ws, "alpha", before);
});

test("U8: a path linked to another project is refused, naming that project and the command that detaches it", () =>
{
    const { box, ws } = workspace();
    const a = folder(box, ws, "alpha");
    must(box, a, ["project", "init", "--name", "alpha", "--no-connect"]);
    const b = folder(box, ws, "beta");
    must(box, b, ["project", "init", "--name", "beta", "--no-connect"]);
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha", b]);
    assert.notEqual(refused.code, 0);
    ...
    const done = must(box, ws, ["project", "unlink", "beta", b, "--force"]);
    assert.ok(done.out.includes(`project "beta" unlinked from ${b}`), done.out);
});

test("U9: neither a path nor --here is refused, and names the listing that answers what may be detached", () =>
{
    const { box, ws, a } = oneLinked();
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha"]);
    assert.notEqual(refused.code, 0);
    ...
    assert.ok(must(box, ws, advertised[1].split(" ")).out.includes(`  ${a}\n`), "the advertised listing did not answer");
    assertUnchanged(ws, "alpha", before);
});

test("U10: --force with no path and no --here falls into the same refusal", () =>
{
    const { box, ws } = oneLinked();
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha", "--force"]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes("project unlink takes the path to detach"), refused.out);
    assertUnchanged(ws, "alpha", before);
});

test("U11: a path and --here together are refused", () =>
{
    const { box, ws, a } = oneLinked();
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha", a, "--here"]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes("project unlink takes one of <path> or --here"), refused.out);
    assertUnchanged(ws, "alpha", before);
});

test("U12: with no slug, --here infers it the way every read verb does", () =>
{
    const { box, ws, a, b } = twoLinked();
    const done = must(box, b, ["project", "unlink", "--here"]);
    assert.ok(done.out.includes(`project "alpha" unlinked from ${b}`), done.out);
    assert.ok(done.out.includes(`  ${a}\n`), done.out);
    assertPruned(ws, "alpha", b);
});

test("U13: with no slug, outside any project, the inference is refused", () =>
{
    const { box, ws } = oneLinked();
    const out = join(box.root, "out");
    mkdirSync(out, { recursive: true });
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, out, ["project", "unlink", "--here"]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes("not inside a registered project"), refused.out);
    assertUnchanged(ws, "alpha", before);
});

test("U14: --here where no linked path contains this directory is refused, naming the linked paths", () =>
{
    const { box, ws, a } = oneLinked();
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "alpha", "--here"]);
    assert.notEqual(refused.code, 0);
...
test("U15: a registered slug with nothing linked to it says so", () =>
{
    const { box, ws, a } = oneLinked();
    halfRegister(ws, "ghost");
    const refused = selfIn(box, ws, ["project", "unlink", "ghost", a]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes(`"${a}" is linked to project "alpha", not "ghost"`), refused.out);
    const elsewhere = selfIn(box, ws, ["project", "unlink", "ghost", ws]);
...
test("U15b: an unregistered slug is refused before anything is read", () =>
{
    const { box, ws, a } = oneLinked();
    const before = snapshot(ws, "alpha");
    const refused = selfIn(box, ws, ["project", "unlink", "nosuch", a]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes('unknown project "nosuch"'), refused.out);
    assertUnchanged(ws, "alpha", before);
});

test("U16: the verdicts are refolded — the detached repository stops being one of the repositories asked", () =>
{
    const { box, ws, a, b } = twoLinked();
    ...
    must(box, a, ["report", work, "done somewhere else", "--evidence", `commit:${hash}`]);
    ...
    must(box, ws, ["project", "unlink", "alpha", b]);
...
test("U16b: a hash already settled stays settled — detaching a repository never unsettles a verified verdict", () =>
{
    const { box, ws, a, b } = twoLinked();
    const work = workIdIn(must(box, a, ["work", "add", "ship it"]).out);
...
test("U17: the listing's advertised link command puts the detached checkout back", () =>
{
    const { box, ws, a } = oneLinked();
    const a2 = worktreeOf(box, a, "alpha2");
    must(box, ws, ["project", "link", "alpha", a2]);
    const done = must(box, a2, ["project", "unlink", "alpha", "--here"]);
    const advertised = done.out.match(/`self (project link alpha --here)`/);
    assert.ok(advertised !== null, done.out);
    const back = must(box, a2, advertised[1].split(" "));
    ...
    assert.ok(must(box, ws, ["project", "link", "alpha"]).out.includes("2 linked paths"), "the round trip did not restore the link");
});

test("U17b: a detached second repository re-links through the --force the link refusal names", () =>
{
    const { box, ws, b } = twoLinked();
    const done = must(box, b, ["project", "unlink", "alpha", "--here"]);
    const advertised = done.out.match(/`self (project link alpha --here)`/);
    assert.ok(advertised !== null, done.out);
    ...
    const guarded = selfIn(box, b, advertised[1].split(" "));
    assert.notEqual(guarded.code, 0);
    assert.ok(guarded.out.includes("pass --force to link it as well"), guarded.out);
    const back = must(box, b, [...advertised[1].split(" "), "--force"]);
    assert.ok(back.out.includes(`project "alpha" linked to ${b}`), back.out);
    assert.ok(must(box, ws, ["project", "link", "alpha"]).out.includes("2 linked paths"), "the round trip did not restore the link");
});

test("U18: a sibling worktree of the same repository keeps answering, and the command says so", () =>
{
    const { box, ws, a } = oneLinked();
    const a2 = worktreeOf(box, a, "alpha2");
    must(box, ws, ["project", "link", "alpha", a2]);
    const done = must(box, ws, ["project", "unlink", "alpha", a]);
...
test("U19: a marker naming another project is left where it is", () =>
{
    const { box, ws } = workspace();
    const a = folder(box, ws, "alpha");
    must(box, a, ["project", "init", "--name", "alpha", "--no-connect"]);
    const home = folder(box, ws, "beta");
    must(box, home, ["project", "init", "--name", "beta", "--no-connect"]);
    ...
    must(box, ws, ["project", "link", "alpha", shared, "--force"]);
    must(box, ws, ["project", "link", "beta", shared, "--force"]);
    assert.equal(JSON.parse(readFileSync(marker(shared), "utf8")).project, "beta");
    const done = must(box, ws, ["project", "unlink", "alpha", shared]);
    assert.ok(done.out.includes(`project "alpha" unlinked from ${shared}`), done.out);
    assert.ok(!done.out.includes("marker there is gone"), done.out);
    assert.equal(JSON.parse(readFileSync(marker(shared), "utf8")).project, "beta");
});

test("U22: --here where a folder link and a checkout inside it both contain this directory takes the nearer one", () =>
{
    const { box, ws, a } = oneLinked();
    ...
    must(box, ws, ["project", "link", "alpha", f, "--force"]);
    must(box, ws, ["project", "link", "alpha", y]);
    const done = must(box, y, ["project", "unlink", "--here"]);
...
test("U20: a named slug and a named path work from outside every project", () =>
{
    const { box, ws, a, b } = twoLinked();
    const out = join(box.root, "out");
    mkdirSync(out, { recursive: true });
    const done = must(box, out, ["project", "unlink", "alpha", b]);
    assert.ok(done.out.includes(`project "alpha" unlinked from ${b}`), done.out);
    assert.ok(done.out.includes(`  ${a}\n`), done.out);
    assertPruned(ws, "alpha", b);
});

test("U21: the project's own record is untouched — no event, and the registry byte-identical", () =>
{
    const { box, ws, a, b } = twoLinked();
    must(box, a, ["work", "add", "something to log"]);

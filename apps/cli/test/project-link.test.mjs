...
async function assertLinked(box, ws, dir, slug, done)
{
    ...
    assert.match((await must(box, dir, ["status"])).out, new RegExp(`^${slug} `));
}
...
test("L1: link with no state directory and no marker links cleanly", async () =>
{
    const { box, ws } = await workspace();
    const dir = folder(box, ws, "ghost");
    halfRegister(ws, "ghost");
    const done = await must(box, dir, ["project", "link", "ghost", "--here"]);
    await assertLinked(box, ws, dir, "ghost", done);
});

test("L2: link with no state directory but a marker present relinks after a prior crash", async () =>
{
    const { box, ws } = await workspace();
    const dir = folder(box, ws, "ghost");
    halfRegister(ws, "ghost");
...
test("L3: link with the state directory present and no marker links cleanly, as today", async () =>
{
    const { box, ws } = await workspace();
    const owning = folder(box, ws, "alpha");
    await must(box, owning, ["project", "init", "--no-connect"]);
    const dir = folder(box, ws, join("elsewhere", "alpha"));
    const done = await must(box, dir, ["project", "link", "alpha", "--here"]);
    await assertLinked(box, ws, dir, "alpha", done);
});

test("L4 (and L18): link with both present replaces the prior link when the repository differs, as today — no --force", async () =>
{
    const { box, ws } = await workspace();
...
test("L5: no arguments inside a linked checkout reads: slug, the path, marked as this directory, nothing written", async () =>
{
    const { box, ws, a } = await registered();
    const before = snapshot(ws, "alpha");
    const read = await must(box, a, ["project", "link"]);
...
test("L6: no arguments inside an unlinked worktree of the repository reads, and says this directory is not linked", async () =>
{
    const { box, ws, a } = await registered();
    const a2 = worktreeOf(box, a, "alpha2");
    const before = snapshot(ws, "alpha");
    const read = await must(box, a2, ["project", "link"]);
...
test("L7: no arguments inside a different repository carrying the slug's marker reads and moves nothing (the incident)", async () =>
{
    const { box, ws, a, b } = await registered();
    writeFileSync(join(b, ".self"), JSON.stringify({ project: "alpha" }) + "\n");
    const before = snapshot(ws, "alpha");
    const read = await must(box, b, ["project", "link"]);
    assert.ok(read.out.includes('project "alpha" is linked on this machine to:'), read.out);
    assert.ok(read.out.includes(`  ${a}\n`), read.out);
...
test("L8: no arguments outside any project is refused, nothing written", async () =>
{
    const { box, ws } = await registered();
    const plain = join(ws, "plain");
    mkdirSync(plain);
    const before = linksText(ws);
    const read = await selfIn(box, plain, ["project", "link"]);
...
test("L9: a slug with no linked path on this machine reads as such and names the write", async () =>
{
    const { box, ws } = await registered();
    halfRegister(ws, "ghost");
    const before = linksText(ws);
    const read = await must(box, ws, ["project", "link", "ghost"]);
...
test("L11: a path already linked is a no-op write with the same receipt", async () =>
{
    const { box, ws, a } = await registered();
    const before = linksText(ws);
    const done = await must(box, ws, ["project", "link", "alpha", a]);
    assert.ok(done.out.includes(`project "alpha" linked to ${a}`), done.out);
    assert.equal(linksText(ws), before);
});

test("L12: another worktree of a linked repository links without --force", async () =>
{
    const { box, ws, a } = await registered();
    const a2 = worktreeOf(box, a, "alpha2");
    const done = await must(box, ws, ["project", "link", "alpha", a2]);
...
test("L13: a different repository is refused without --force, nothing written", async () =>
{
    const { box, ws, a, b } = await registered();
    const before = snapshot(ws, "alpha");
    const refused = await selfIn(box, ws, ["project", "link", "alpha", b]);
...
test("L14: --force links the second repository, says old and new, and refolds the verdicts across both", async () =>
{
    ...
    const done = await must(box, ws, ["project", "link", "alpha", b, "--force"]);
...
test("L15: with no link standing, a path links without --force", async () =>
{
    const { box, ws, b } = await registered();
    halfRegister(ws, "ghost");
    const done = await must(box, ws, ["project", "link", "ghost", b]);
    assert.ok(done.out.includes(`project "ghost" linked to ${b}`), done.out);
    assert.ok(!done.out.includes("was linked to"), done.out);
});

test("L16: a folder holding repositories links with --force, and the fold judges through its children", async () =>
{
    const { box, ws, a } = await registered();
    ...
    assert.notEqual(refused.code, 0);
    const done = await must(box, ws, ["project", "link", "alpha", f, "--force"]);
    assert.ok(done.out.includes(`now linked to ${a}, ${realpathSync(f)}`), done.out);
    assert.equal(verdicts(ws, "alpha")[hash], "settled");
});

test("L17: --force where nothing needs it is accepted with the same receipt", async () =>
{
    const { box, ws, a, b } = await registered();
    halfRegister(ws, "ghost");
    const fresh = await must(box, ws, ["project", "link", "ghost", b, "--force"]);
    assert.ok(fresh.out.includes(`project "ghost" linked to ${b}`), fresh.out);
    const a2 = worktreeOf(box, a, "alpha2");
    const same = await must(box, ws, ["project", "link", "alpha", a2, "--force"]);
    assert.ok(same.out.includes(`project "alpha" linked to ${a2}`), same.out);
    assert.ok(!same.out.includes("was linked to"), same.out);
});

test("L19: --force with nowhere to write is refused", async () =>
{
    const { box, ws, a } = await registered();
    const before = linksText(ws);
    const refused = await selfIn(box, a, ["project", "link", "--force"]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes("--force applies to a write — name the path to link, or pass --here"), refused.out);
    assert.equal(linksText(ws), before);
});

test("L20: a path and --here together are refused", async () =>
{
    const { box, ws, a, b } = await registered();
    const before = linksText(ws);
    const refused = await selfIn(box, a, ["project", "link", "alpha", b, "--here"]);
    assert.notEqual(refused.code, 0);
    assert.ok(refused.out.includes("project link takes one of <path> or --here"), refused.out);
    assert.equal(linksText(ws), before);
});

test("L21: the remedy the duplicate-init refusal prints links the checkout it is run in", async () =>
{
    const { box, ws, a } = await registered();
    const a2 = worktreeOf(box, a, "alpha2");
    const refused = await selfIn(box, a2, ["project", "init", "--no-connect"]);
    assert.notEqual(refused.code, 0);
    const advertised = refused.out.match(/`self (project link alpha --here)`/);
    assert.ok(advertised !== null, refused.out);
    const done = await must(box, a2, advertised[1].split(" "));
    await assertLinked(box, ws, a2, "alpha", done);
});

...
async function terminal(args = ["context"])
{
    return (await must(box, demo, [...args, "--pretty"])).out;
}

async function piped(args = ["context"])
{
    return (await must(box, demo, [...args, "--plain"])).out;
}
...
const rule = entityIn((await must(box, demo, ["state", "add", "a standing rule", "--exposure", "full"])).out);
await must(box, demo, ["state", "place", rule, "--exposure", "index", "--why", "too broad for full", "--proposed"]);

const proposedDecision = (await must(box, demo, ["decide", "a rule nobody confirmed", "--why", "it needs a ruling", "--proposed"]))
...
const proposal = workIdIn((await must(box, demo, ["work", "propose", "a proposed direction", "--objective", objective,
...
test("a work proposal carries its accept command, which the row text never swallows", async () =>
{
    const shown = await terminal();
...
{
    assert.ok((await terminal()).includes(`self decide confirm ${proposedDecision}`));
});

test("a unit blocked on a decision carries the command that shows it", async () =>
{
    const shown = await terminal();
...
test("the band is legible with no colour at all", async () =>
{
    const shown = await terminal();
...
test("waiting rows lead the page, ahead of the work roll-up", async () =>
{
    const shown = await terminal();
...
test("status shows the same band with the same commands", async () =>
{

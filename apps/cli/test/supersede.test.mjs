...
// terminal state.
function work(outcome)
{
    return workIdIn(must(box, demo, ["work", "add", outcome]).out);
}

function objective(outcome)
{
    return entityIn(must(box, demo, ["objective", "add", outcome]).out);
}

function entity(text)
{
    return must(box, demo, ["state", "add", text]).out.match(/\be-[0-9a-z]{5}\b/)[0];
}
...
test("K7: decide --supersedes on a work id names work add --supersedes", () =>
{
    const unit = work("K7 outcome");
    const refused = self(["decide", "K7 text", "--supersedes", unit]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /is a work record/);
    assert.match(refused.out, /self work add "<outcome>" --supersedes <id>/);
});

test("K8: objective add --supersedes on a work id names work add --supersedes", () =>
{
    const unit = work("K8 outcome");
    const refused = self(["objective", "add", "K8 outcome", "--supersedes", unit]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /is a work record/);
    assert.match(refused.out, /self work add "<outcome>" --supersedes <id>/);
});

test("K9: milestone add --supersedes on a decision id names decide --supersedes", () =>
{
    const parent = objective("K9 objective");
    const decision = idIn(must(box, demo, ["decide", "K9 decision"]).out);
    const refused = self(["milestone", "add", "K9 checkpoint", "--objective", parent, "--exit", "c", "--supersedes", decision]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /is a decision record/);
    assert.match(refused.out, /self decide "<text>" --supersedes <id>/);
});

test("K10: convention add --supersedes on an objective id names objective add --supersedes", () =>
{
    const parent = objective("K10 objective");
    const refused = self(["convention", "add", "K10 rule", "--supersedes", parent]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /is an? objective record/);
    assert.match(refused.out, /self objective add "<outcome>" --supersedes <id>/);
});

test("K11: work add --supersedes on a decision id names decide --supersedes", () =>
{
    const decision = idIn(must(box, demo, ["decide", "K11 decision"]).out);
    const refused = self(["work", "add", "K11 outcome", "--supersedes", decision, "--why", "w"]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /is a decision record/);
    assert.match(refused.out, /self decide "<text>" --supersedes <id>/);
});

test("K12: state add --supersedes on a convention id names convention add --supersedes", () =>
{
    const convention = idIn(must(box, demo, ["convention", "add", "K12 rule"]).out);
    const refused = self(["state", "add", "K12 text", "--supersedes", convention]);
    assert.notEqual(refused.code, 0);
...
test("B1: superseding a done unit refuses — retirement is not a transition from done", () =>
{
    const unit = work("B1 outcome");
    must(box, demo, ["work", "done", unit, "--report", "B1 verifiably happened"]);
    const refused = self(["work", "add", "B1 corrected outcome", "--supersedes", unit, "--why", "w"]);
    assert.notEqual(refused.code, 0);
...
test("B3: --supersedes naming an id nothing answers to refuses, naming the id", () =>
{
    const unit = self(["work", "add", "B3 outcome", "--supersedes", "w-zzzzz", "--why", "w"]);
    assert.notEqual(unit.code, 0);
    assert.match(unit.out, /unknown work id "w-zzzzz"/);
    const free = self(["state", "add", "B3 text", "--supersedes", "e-zzzzz"]);
    assert.notEqual(free.code, 0);
    assert.match(free.out, /unknown entity "e-zzzzz"/);
    const decision = self(["decide", "B3 text", "--supersedes", "01zzzzz"]);
    assert.notEqual(decision.code, 0);
    assert.match(decision.out, /01zzzzz is not a decision/);
});

test("B4: superseding a unit without --why refuses — the retirement keeps its reason", () =>
{
    const unit = work("B4 outcome");
    const refused = self(["work", "add", "B4 corrected outcome", "--supersedes", unit]);
    assert.notEqual(refused.code, 0);
...
test("B7: --why with nothing superseded is refused, never dropped", () =>
{
    const refused = self(["work", "add", "B7 outcome", "--why", "a reason for nothing"]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /--supersedes <work-id>/);
    assert.ok(!must(box, demo, ["work"]).out.includes("B7 outcome"), "the unit was created with its --why dropped");
});

test("B6: state add --supersedes on a work id names work add --supersedes", () =>
{
    const unit = work("B6 outcome");
    const refused = self(["state", "add", "B6 text", "--supersedes", unit]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /is a work record/);

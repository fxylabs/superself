...
{
    return idIn(must(box, demo, ["decide", text]).out);
}
...
test("a plan of three withdrawals refuses without a terminal, discloses all three, and records nothing", () =>
{
    const ids = ["A1 the duplicate", "A2 the superseded scope", "A3 the spent scope"].map(decided);
    ...
    const refused = selfIn(box, demo, ["apply", path]);
...
{
    const ids = ["B1 the duplicate", "B2 the superseded scope", "B3 the spent scope"].map(decided);
    ...
    assert.doesNotMatch(must(box, demo, ["context"]).out, /B1 the duplicate/);
});

test("a wrong answer at the terminal records nothing from the plan", async () =>
{
    const ids = ["C1 a live decision", "C2 another live decision"].map(decided);
...
{
    const ids = ["D1 the duplicate", "D2 the other duplicate"].map(decided);
...
test("a line that records rather than destroys refuses the whole plan, with nothing written", () =>
{
    const live = decided("E1 a decision the plan leaves alone");
    ...
    assert.match(must(box, demo, ["state", "show", live]).out, /confirmed/);
});

test("a plan the store's own verb refuses names the line, and the earlier lines are not written", () =>
{
    const live = decided("F1 a decision that stays");
...
test("a plan naming one record twice is refused before anybody is asked", () =>
{
    const live = decided("G1 a decision named twice");
...
test("a plan mixing record kinds says how many records, not how many decisions", () =>
{
    const decision = decided("H1 a decision that goes");
    const unit = must(box, demo, ["work", "add", "H2 an outcome that is given up"]).out.match(/\bw-[0-9a-z]{5}\b/)[0];
...
test("a line no command dispatches, an unreadable file, and an empty plan are each named", () =>
{
    const unknown = selfIn(box, demo, ["apply", plan("unknown.txt", ["nope whatever"])]);
    assert.equal(unknown.code, 1);
    assert.match(unknown.out, /line 1 was refused/);
    assert.match(unknown.out, /`self nope` is not a verb a plan runs/);

    const missing = selfIn(box, demo, ["apply", join(demo, "not-written-yet.txt")]);
    assert.equal(missing.code, 1);
    assert.match(missing.out, /no plan to read at/);

    const empty = selfIn(box, demo, ["apply", plan("notes.txt", ["# nothing but a note", ""])]);
...
{
    const live = decided("I1 a decision with a quoted reason");
...
test("an unclosed quote is refused by naming the line it opened on", () =>
{
...
test("a line that only reads is refused, so nothing rides along inside a reviewed set", () =>
{
    const live = decided("K1 a decision the plan retires");
...
test("an alias verb's supersession is a line a plan runs", () =>
{
    must(box, demo, ["alias", "add", "note", "--exposure", "search"]);
    const first = must(box, demo, ["note", "add", "M1 the note that is replaced"]).out.match(/\be-[0-9a-z]{5}\b/);
...
test("a plan cannot apply another plan", () =>
{
    const live = decided("L1 a decision behind a nested plan");
    const inner = plan("inner.txt", [`decide retract ${live} --why "spent"`]);
    const refused = selfIn(box, demo, ["apply", plan("outer.txt", [`apply ${inner}`])]);
    assert.equal(refused.code, 1);
...
{
    const ids = ["N1 the first record", "N2 the second record"].map(decided);
...
{
    const ids = ["O1 one", "O2 two", "O3 three"].map(decided);
...
{
    const ids = ["P1 one", "P2 two"].map(decided);
    const path = plan("archived.txt", ids.map((id) => `decide retract ${id} --why "spent"`));
    must(box, ws, ["project", "archive", "demo", "--why", "set aside while the plan sat"]);
    const before = events().length;
    const refused = await approvedIn(box, demo, ["apply", path], ids.join(" "));
    assert.equal(refused.code, 1);
...
test("a line that changes something other than a record is refused before it runs", () =>
{
    const live = decided("Q1 a decision the plan retires");
    ...
    const remote = selfIn(box, demo, ["apply", plan("remote.txt", [
    ...
    const theme = selfIn(box, demo, ["apply", plan("theme.txt", [
        `decide retract ${live} --why "spent"`,
        "theme cyan"
    ])]);
...
test("a supersession discloses the record it would write, not only the one it retires", () =>
{
    const rule = idIn(must(box, demo, ["convention", "add", "R1 the rule as first written"]).out);
    const decision = decided("R2 the decision as first taken");
    must(box, demo, ["alias", "add", "memo", "--exposure", "search"]);
    const memo = must(box, demo, ["memo", "add", "R3 the memo as first written"]).out.match(/\be-[0-9a-z]{5}\b/)[0];
    const refused = selfIn(box, demo, ["apply", plan("successors.txt", [
...
test("a retirement's reason is spelled retired, in a plan and in a single command", () =>
{
    const unit = workIdIn(must(box, demo, ["work", "add", "S1 an outcome that moved"]).out);
    const single = selfIn(box, demo, ["work", "retire", unit, "--why", "the outcome moved to another unit"]);
    ...
    const inPlan = selfIn(box, demo, ["apply",

...
function statusLine()
{
    return must(box, demo, ["status"]).out;
}

function log()
{
    return must(box, demo, ["log", "-n", "500"]).out;
}

/* ── A. the creation event × the fold ──────────────────────────────── */

test("A1: a store whose only unit is a `work.created` one lists no open work", () =>
{
    legacyUnit("A1: an outcome from before entities");
    assert.equal(must(box, demo, ["work"]).out.trim(), "no open work");
});

test("A2: `context` on the same store folds, with no work section", () =>
{
    const shown = context();
...
test("A3: a unit whose creation event is `entity.confirmed` lists as it always did", () =>
{
    const work = workIdIn(must(box, demo, ["work", "add", "A3: an outcome recorded today"]).out);
    assert.ok(must(box, demo, ["work"]).out.includes(work));
});

test("A4: a legacy unit beside a native one leaves only the native one", () =>
{
    const legacy = legacyUnit("A4: the outcome from before");
    const native = workIdIn(must(box, demo, ["work", "add", "A4: the outcome from today"]).out);
...
test("A5: `work show` on a legacy id refuses as an unknown id, pointing at search", () =>
{
    const legacy = legacyUnit("A5: an outcome nobody can show");
    const refused = selfIn(box, demo, ["work", "show", legacy]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, new RegExp(`unknown work id "${legacy}"`));
    assert.match(refused.out, /self search/);
});

test("A6: a project whose work history is entirely legacy folds without an exception", () =>
{
    const only = join(ws, "only");
    mkdirSync(only, { recursive: true });
    git(box, only, ["init", "-q", "-b", "main"]);
    must(box, only, ["project", "init", "--name", "only", "--desc", "legacy only"]);
    ...
    const shown = must(box, demo, ["context", "--project", "only"]);
    assert.equal(shown.code, 0);
    assert.ok(!shown.out.includes("A6: the only outcome"), `a legacy unit reached the render:\n${shown.out}`);
});

test("A7: a proposal recorded as `entity.proposed` still becomes a unit on accept", () =>
{
    const id = nativeProposal("A7: an outcome proposed by today's verb");
    must(box, demo, ["work", "accept", id]);
    assert.ok(must(box, demo, ["work"]).out.includes(id));
});

/* ── B. legacy proposals × answering them ──────────────────────────── */

test("B1: a `work.proposed` line is in no proposal list and no waiting count", () =>
{
    const before = Number(statusLine().match(/waiting on you: (\d+)/)[1]);
    legacyLine("work.proposed", { outcome: "B1: an outcome proposed before the cutover", objective, ...BRIEF });
    assert.ok(!context().includes("B1: an outcome proposed before the cutover"));
    assert.equal(Number(statusLine().match(/waiting on you: (\d+)/)[1]), before);
});

test("B2: accepting a legacy proposal by its event id is refused as no proposal", () =>
{
    const id = legacyLine("work.proposed", { outcome: "B2: an outcome nobody can accept", objective, ...BRIEF });
    const refused = selfIn(box, demo, ["work", "accept", id]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, new RegExp(`no work proposal matches "${id}"`));
});

test("B3: declining a legacy proposal by its event id is refused the same way", () =>
{
    const id = legacyLine("work.proposed", { outcome: "B3: an outcome nobody can decline", objective, ...BRIEF });
    const refused = selfIn(box, demo, ["work", "decline", id, "--why", "its premises were removed"]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, new RegExp(`no work proposal matches "${id}"`));
});

test("B4: a legacy proposal an `entity.confirmed` line answered still becomes no unit", () =>
{
...
test("B5: a proposal recorded today keeps its short-id waiting row", () =>
{
    const id = nativeProposal("B5: an outcome waiting on a person");
    assert.match(id, /^w-[0-9a-z]{5}$/);
    assert.ok(context().includes(`- work proposal ${id}: B5: an outcome waiting on a person`), context());
});

test("B6: two legacy proposals written in one millisecond leave no prefix to collide", () =>
{
    ...
    assert.equal(selfIn(box, demo, ["work", "accept", millisecond]).code, 1);
});

test("B7: a legacy proposal's text is found by no search", () =>
{
    legacyLine("work.proposed", { outcome: "B7: an outcome no search finds", objective, ...BRIEF });
    assert.equal(must(box, demo, ["search", "B7: an outcome no search finds"]).out.trim(), "no matches");
});

/* ── C. legacy execution transitions × the fold ────────────────────── */

test("C1: `work.created` followed by `work.started` folds to no unit", () =>
{
...
test("C2: `work.blocked` on a legacy unit raises no waiting row", () =>
{
    const work = legacyUnit("C2: an outcome parked on a ruling");
    legacyLine("work.blocked", { work, on: "decision", why: "C2: pricing undecided" });
    assert.ok(!context().includes("C2: pricing undecided"), context());
});

test("C3: `work.done` on a legacy unit closes nothing, because there is nothing to close", () =>
{
    const work = legacyUnit("C3: an outcome an older binary closed");
    legacyLine("work.done", { work });
    assert.match(statusLine(), /work: [^\n]*\b0 done\b/);
});

test("C4: `work.retired` on a legacy unit leaves no retired unit", () =>
{
    const work = legacyUnit("C4: an outcome given up before the cutover");
    legacyLine("work.retired", { work, why: "C4: superseded" });
    assert.ok(!must(box, demo, ["work"]).out.includes(work));
    assert.ok(!context().includes("C4: superseded"));
});

test("C5: `work.linked` from a legacy unit contributes to no objective", () =>
{
    const work = legacyUnit("C5: an outcome linked before the cutover");
    legacyLine("work.linked", { work, objective });
    const shown = must(box, demo, ["objective", "show", objective]).out;
    assert.ok(!shown.includes(work), `a legacy unit is still cited as a contribution:\n${shown}`);
});

test("C6: an entity unit moved by `entity.started` reads in-progress as it always did", () =>
{
    const work = workIdIn(must(box, demo, ["work", "add", "C6: an outcome moved today"]).out);
    must(box, demo, ["work", "start", work]);
    assert.match(must(box, demo, ["work", "show", work]).out, /- Status: active/);
});

test("C7: a `work.blocked` line merged onto a native unit is ignored rather than refused", () =>
{
    const work = workIdIn(must(box, demo, ["work", "add", "C7: an outcome an old clone blocked"]).out);
    legacyLine("work.blocked", { work, on: "dependency", why: "C7: the upstream fix" });
    const shown = must(box, demo, ["work", "show", work]);
    assert.equal(shown.code, 0);
    assert.match(shown.out, /- Status: next/);
    assert.ok(!shown.out.includes("C7: the upstream fix"), shown.out);
});

test("C8: a live milestone linked only to legacy units renders no work, and its criteria do not move", () =>
{
    const milestone = must(box, demo, ["milestone", "add", "C8: suite green", "--objective", objective, "--exit", "C8: tests pass"])
        .out.match(/\bm-[0-9a-z]{5}\b/)[0];
    const before = must(box, demo, ["milestone", "show", milestone]).out;
    const work = legacyUnit("C8: an outcome the milestone counted");
    legacyLine("work.linked", { work, milestone });
    const after = must(box, demo, ["milestone", "show", milestone]).out;
    ...
    assert.ok(must(box, demo, ["milestone"]).out.includes(`${milestone}  unstarted  C8: suite green — no work linked yet [no work linked]`),
        must(box, demo, ["milestone"]).out);
});

/* ── E. reports and receipts that name a dropped unit ──────────────── */

test("E1: a report naming a legacy unit attaches to nothing, as a report naming an unknown unit does", () =>
{
    const work = legacyUnit("E1: an outcome someone reported on");
    legacyLine("report.added", { text: "E1: the report from before" }, { work });
    const shown = context();
    assert.ok(!must(box, demo, ["work"]).out.includes(work), "the reported-on legacy unit is listed");
    assert.ok(!shown.includes("E1: the report from before"), shown);
});

test("E2: the same report event still prints on `self log`", () =>
{
    assert.match(log(), /E1: the report from before/);
});

test("E3: a report naming a native unit attaches as it always did", () =>
{
    const work = workIdIn(must(box, demo, ["work", "add", "E3: an outcome reported on today"]).out);
    must(box, demo, ["report", work, "E3: the report from today"]);
    assert.match(must(box, demo, ["work", "show", work]).out, /E3: the report from today/);
});

test("E4: a review receipt naming a legacy unit folds to nothing without an exception", () =>
{
    ...
    assert.equal(selfIn(box, demo, ["fold"]).code, 0);
    assert.equal(selfIn(box, demo, ["context"]).code, 0);
    assert.ok(!must(box, demo, ["work"]).out.includes(work), "the reviewed legacy unit is listed");
});

/* ── F. `self log`, the surface the dropped records stay readable on ── */

test("F1: a `work.created` line prints as a human-readable row", () =>
{
...
test("F2: `work.proposed`, `work.required` and `work.covered` all print, none of them raw", () =>
{
...
test("F3: an evidence-free `work.done` prints, and closes no unit", () =>
{
    const work = workIdIn(must(box, demo, ["work", "add", "F3: an outcome an older binary closed"]).out);
    legacyLine("work.done", { work });
...
test("F4: legacy and native lines print together, in the order they were written", () =>
{
...
test("F5: a legacy id resolves to no record, and refuses rather than throwing", () =>
{
    const work = legacyUnit("F5: an outcome with a history nobody can read");
    const refused = selfIn(box, demo, ["state", "show", work, "--history"]);
    assert.equal(refused.code, 1);
    assert.match(refused.out, new RegExp(`unknown entity "${work}"`));
    assert.equal(must(box, demo, ["search", "F5: an outcome with a history nobody can read"]).out.trim(), "no matches");
});

/* ── R. the process events, which are current and stay ─────────────── */

const runBox = machine();
const { ws: runWs, demo: runDemo } = demoWorkspace(runBox);

test("R1: `work.run-started` puts a native unit's process at running", () =>
{
    const work = workIdIn(must(runBox, runDemo, ["work", "add", "R1: an outcome with a run"]).out);
    retireFixture(runBox, runWs, "demo", "work.run-started", { work });
    assert.match(must(runBox, runDemo, ["work", "show", work]).out, /- Process: running at /);
});

test("R2: `work.run-exited` puts it at exited, with the code it carried", () =>
{
    const work = workIdIn(must(runBox, runDemo, ["work", "add", "R2: an outcome whose run ended"]).out);
    retireFixture(runBox, runWs, "demo", "work.run-started", { work });
    retireFixture(runBox, runWs, "demo", "work.run-exited", { work, code: 0 });
    assert.match(must(runBox, runDemo, ["work", "show", work]).out, /- Process: exited \(code 0\) at /);
});

test("R3: a run event naming a legacy unit attaches to nothing, and is no error", () =>
{
    ...
    assert.equal(selfIn(runBox, runDemo, ["fold"]).code, 0);
    assert.equal(selfIn(runBox, runDemo, ["work", "show", "w-legr3"]).code, 1);
    assert.ok(!must(runBox, runDemo, ["work"]).out.includes("w-legr3"), "a run event revived a dropped unit");
});

test("R4: a run event naming a unit nothing knows is ignored, and is no error", () =>
{
    retireFixture(runBox, runWs, "demo", "work.run-exited", { work: "w-nobod", code: 1 });
    assert.equal(selfIn(runBox, runDemo, ["context"]).code, 0);
    assert.equal(selfIn(runBox, runDemo, ["status"]).code, 0);
});

test("R5: a run event moves the unit's last-event time, so a running unit is not stalled", () =>
{
    const work = workIdIn(must(runBox, runDemo, ["work", "add", "R5: an outcome running for a while"]).out);
    ...
    assert.match(must(runBox, runDemo, ["status"]).out, new RegExp(`${work} looks stalled`));
    retireFixture(runBox, runWs, "demo", "work.run-started", { work });
    assert.doesNotMatch(must(runBox, runDemo, ["status"]).out, new RegExp(`${work} looks stalled`),
        "a unit whose run just started still reads as stalled");
});

test("R6: a run event's branch reaches the unit's page", () =>
{
    const work = workIdIn(must(runBox, runDemo, ["work", "add", "R6: an outcome run on a branch"]).out);

...
function withGuide(text = "a guide too long for a context line\n")
{
    const box = machine();
    const { ws, demo } = demoWorkspace(box);
    writeFileSync(join(demo, "guide.md"), text);
    const artifact = artifactIdIn(must(box, demo, ["artifact", "add", "guide.md"]).out);
...
const reg = machine();
const regPaths = demoWorkspace(reg);

test("1: a plain file registers on its own, and lists with no work unit", () =>
{
    writeFileSync(join(regPaths.demo, "guide.md"), "the guide\n");
    const added = must(reg, regPaths.demo, ["artifact", "add", "guide.md", "--why", "the API guide"]);
    ...
    assert.match(must(reg, regPaths.demo, ["artifact", "list"]).out, new RegExp(`${id}\\s+\\S+\\s+demo\\s+-\\s+guide\\.md`));
});

test("2: a directory registers as one bundle", () =>
{
    const dir = join(regPaths.demo, "docs");
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "README.md"), "# docs\n");
    writeFileSync(join(dir, "detail.md"), "detail\n");
    const id = artifactIdIn(must(reg, regPaths.demo, ["artifact", "add", "docs"]).out);
    assert.match(must(reg, regPaths.demo, ["artifact", "list"]).out, new RegExp(`${id}\\s+\\S+\\s+demo\\s+-\\s+docs/ \\(2 files\\)`));
});

test("3: bytes the project already stores are stored once and referenced twice", () =>
{
    writeFileSync(join(regPaths.demo, "copy.md"), "the guide\n");
    const twin = artifactIdIn(must(reg, regPaths.demo, ["artifact", "add", "copy.md"]).out);
...
test("4: a registration outside any project is refused", () =>
{
    const refused = selfIn(reg, reg.root, ["artifact", "add", "guide.md"]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /not inside a registered project/);
});

test("5: a file over the byte bound is refused", () =>
{
    const huge = join(regPaths.demo, "huge.bin");
    writeFileSync(huge, "");
    truncateSync(huge, 100 * 1024 * 1024 + 1);
    const before = events(regPaths.ws).length;
    const refused = selfIn(reg, regPaths.demo, ["artifact", "add", "huge.bin"]);
    rmSync(huge);
    assert.notEqual(refused.code, 0);
...
test("6: a path that does not exist is refused, and writes no event", () =>
{
    const before = events(regPaths.ws).length;
    const refused = selfIn(reg, regPaths.demo, ["artifact", "add", "nope.md"]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /artifact "nope\.md" does not exist/);
    assert.equal(events(regPaths.ws).length, before);
});

test("7: registering a file is not evidence, so it never opens `work done`", () =>
{
    const work = workIdIn(must(reg, regPaths.demo, ["work", "add", "the flow works"]).out);
    must(reg, regPaths.demo, ["artifact", "add", "guide.md"]);
    const refused = selfIn(reg, regPaths.demo, ["work", "done", work]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, new RegExp(`${work} has no evidence for done`));
});

test("8: a registered artifact opens", () =>
{
    const id = artifactIdIn(must(reg, regPaths.demo, ["artifact", "list"]).out);
    const opened = must(reg, regPaths.demo, ["artifact", "open", id]);
    assert.match(opened.out, new RegExp(`\\(${id}\\) resolves to that path`));
});

test("9: a registered artifact is found by search", () =>
{
    assert.match(must(reg, regPaths.demo, ["artifact", "search", "guide.md"]).out, /guide\.md/);
});

/* ── a record referencing one (10-18) ──────────────────────────────── */

test("10: an id this project stores is recorded on the record, in one event", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    const before = events(ws).length;
    const rule = idIn(must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]).out);
    assert.equal(events(ws).length, before + 1, "an id already stored registers nothing");
    assert.match(must(box, demo, ["state", "show", rule]).out, new RegExp(`artifact: ${artifact}`));
});

test("11: a path registers first and the record is written second", () =>
{
    const box = machine();
    const { ws, demo } = demoWorkspace(box);
    writeFileSync(join(demo, "guide.md"), "the guide\n");
    const before = events(ws).length;
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", "./guide.md"]);
...
test("12: an id nothing recorded is refused, and writes no event", () =>
{
    const { box, ws, demo } = withGuide();
    const before = events(ws).length;
    const refused = selfIn(box, demo, ["convention", "add", "a rule", "--artifact", "a-zzzzz"]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /unknown artifact "a-zzzzz" in project "demo"/);
    assert.equal(events(ws).length, before);
});

test("13: another project's artifact is refused at the project boundary", () =>
{
    const { box, ws, demo } = withGuide();
    const other = join(ws, "other");
    mkdirSync(other, { recursive: true });
    git(box, other, ["init", "-q", "-b", "main"]);
    must(box, other, ["project", "init", "--name", "other", "--desc", "the second project"]);
    writeFileSync(join(other, "elsewhere.md"), "another project's guide\n");
    const theirs = artifactIdIn(must(box, other, ["artifact", "add", "elsewhere.md"]).out);
    const before = events(ws).length;
    const refused = selfIn(box, demo, ["convention", "add", "a rule", "--artifact", theirs]);
    assert.notEqual(refused.code, 0);
...
test("14: an id the project's registry no longer holds is refused", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    ...
    const refused = selfIn(box, demo, ["convention", "add", "a rule", "--artifact", artifact]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, new RegExp(`unknown artifact "${artifact}" in project "demo"`));
});

test("15: a second --artifact is refused by name, with how many times it was passed", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    const before = events(ws).length;
    const refused = selfIn(box, demo, ["convention", "add", "a rule", "--artifact", artifact, "--artifact", artifact]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /--artifact names one artifact and was passed 2 times/);
    assert.equal(events(ws).length, before);
});

test("16: without --artifact a record carries no reference", () =>
{
    const { box, demo } = withGuide();
    const rule = idIn(must(box, demo, ["convention", "add", "a rule with no guide"]).out);
    assert.doesNotMatch(must(box, demo, ["state", "show", rule]).out, /artifact:/);
});

test("17: a registration that lands before the record is refused leaves an artifact nothing points at", () =>
{
    const box = machine();
    const { ws, demo } = demoWorkspace(box);
    writeFileSync(join(demo, "guide.md"), "the guide\n");
    ...
    const before = events(ws).length;
    const refused = selfIn(box, demo, ["convention", "add", "a rule the cap refuses", "--artifact", "./guide.md"]);
    assert.notEqual(refused.code, 0);
    const written = events(ws).slice(before);
    assert.equal(written.length, 1);
    assert.equal(written[0].type, "artifact.registered", "the artifact stands, and the record it was for does not");
    assert.match(must(box, demo, ["artifact", "list"]).out, /guide\.md/);
});

test("18: `state add --artifact` records the same field through the same check", () =>
{
    const { box, demo, artifact } = withGuide();
    const note = entityIdIn(must(box, demo, ["state", "add", "a note", "--artifact", artifact]).out);
    assert.match(must(box, demo, ["state", "show", note]).out, new RegExp(`artifact: ${artifact}`));
    assert.notEqual(selfIn(box, demo, ["state", "add", "another note", "--artifact", "a-zzzzz"]).code, 0);
});

/* ── what the retention cap charges (19-27) ────────────────────────── */

test("19: with room in the tier, a referencing record is admitted", () =>
{
    const { box, demo, artifact } = withGuide();
    setCaps(box, { fullTokens: 4000 });
    const rule = idIn(must(box, demo, ["convention", "add", "a rule", "--artifact", artifact]).out);
    assert.match(must(box, demo, ["state", "show", rule]).out, /placement: project · full/);
});

test("20: the pointer is what a refusal counts, and the number says so", () =>
{
    const { box, demo, artifact } = withGuide();
    const text = "abcdefghij";
    setCaps(box, { fullTokens: text.length });
    const refused = selfIn(box, demo, ["convention", "add", text, "--artifact", artifact]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, new RegExp(`this text adds ${text.length + POINTER} more`));
});

test("21: the reference moves the refusal boundary by exactly the pointer's cost", () =>
{
    const text = "abcdefghij";
    const bare = withGuide();
    setCaps(bare.box, { fullTokens: text.length });
    assert.equal(selfIn(bare.box, bare.demo, ["convention", "add", text]).code, 0,
        "at a cap the text exactly fits, an unreferencing record is admitted");
    const tight = withGuide();
    setCaps(tight.box, { fullTokens: text.length });
    assert.notEqual(selfIn(tight.box, tight.demo, ["convention", "add", text, "--artifact", tight.artifact]).code, 0,
        "the same text with a reference does not");
    const roomy = withGuide();
    setCaps(roomy.box, { fullTokens: text.length + POINTER });
    assert.equal(selfIn(roomy.box, roomy.demo, ["convention", "add", text, "--artifact", roomy.artifact]).code, 0,
        "and it fits again once the cap covers the pointer");
});

test("22: the artifact's own length is not counted — a long guide costs the pointer", () =>
{
    const text = "abcdefghij";
    const { box, demo, artifact } = withGuide("x".repeat(12_000) + "\n");
    setCaps(box, { fullTokens: text.length + POINTER });
    assert.equal(selfIn(box, demo, ["convention", "add", text, "--artifact", artifact]).code, 0);
});

test("23: --demote frees the room a referencing record needs", () =>
{
    const text = "abcdefghij";
    const { box, demo, artifact } = withGuide();
    setCaps(box, { fullTokens: text.length + POINTER, indexTokens: 4000 });
    const first = idIn(must(box, demo, ["convention", "add", text]).out);
    const refused = selfIn(box, demo, ["convention", "add", text, "--artifact", artifact]);
    assert.notEqual(refused.code, 0);
    assert.equal(selfIn(box, demo, ["convention", "add", text, "--artifact", artifact, "--demote", first]).code, 0);
});

test("24: an index-exposure record is charged the pointer against the index cap", () =>
{
    const text = "abcdefghij";
    const { box, demo, artifact } = withGuide();
    setCaps(box, { indexTokens: text.length });
    const refused = selfIn(box, demo, ["state", "add", text, "--exposure", "index", "--artifact", artifact]);
    assert.notEqual(refused.code, 0);
    ...
    const note = entityIdIn(must(box, demo, ["state", "add", text, "--exposure", "index", "--artifact", artifact]).out);
    assert.match(must(box, demo, ["context"]).out, new RegExp(`- .*${text}.* — see \`self artifact open ${artifact}\``));
    assert.match(must(box, demo, ["state", "show", note]).out, /placement: project · index/);
});

test("25: a search-exposure record has no cap, and its pointer renders in `self search`", () =>
{
    const { box, demo, artifact } = withGuide();
    setCaps(box, { fullTokens: 1, indexTokens: 1 });
    const note = entityIdIn(must(box, demo, ["state", "add", "a searchable note", "--exposure", "search", "--artifact", artifact]).out);
    const found = must(box, demo, ["search", "--all", "a searchable note"]).out;
...
test("26: a supersession credits the predecessor's pointer as well as its text", async () =>
{
    ...
    const { box, demo, artifact } = withGuide();
    setCaps(box, { fullTokens: text.length + POINTER });
    const first = idIn(must(box, demo, ["convention", "add", text, "--artifact", artifact]).out);
...
test("27: the confirm-time check counts the pointer too", () =>
{
    const text = "abcdefghij";
    const { box, demo, artifact } = withGuide();
    setCaps(box, { fullTokens: text.length });
    ...
    const proposal = entityIdIn(must(box, demo,
        ["state", "add", text, "--exposure", "full", "--artifact", artifact, "--proposed"]).out);
    const refused = selfIn(box, demo, ["state", "confirm", proposal]);
    assert.notEqual(refused.code, 0, "a proposal passes the add and the confirm is where the tier is judged");
    setCaps(box, { fullTokens: text.length + POINTER });
    assert.equal(selfIn(box, demo, ["state", "confirm", proposal]).code, 0);
});

/* ── what a reader sees (28-38, 43, 44) ────────────────────────────── */

test("28: context renders the rule and a pointer to its guide", () =>
{
    const { box, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]);
    assert.match(must(box, demo, ["context"]).out,
        new RegExp(`- \\[convention\\] API design follows the guide — see \`self artifact open ${artifact}\``));
});

test("29: a workspace-scoped rule renders its pointer elsewhere, and the artifact opens there", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "every project follows the guide", "--artifact", artifact, "--workspace"]);
    const other = join(ws, "other");
    mkdirSync(other, { recursive: true });
    git(box, other, ["init", "-q", "-b", "main"]);
    must(box, other, ["project", "init", "--name", "other", "--desc", "the second project"]);
    assert.match(must(box, other, ["context"]).out, new RegExp(`— see \`self artifact open ${artifact}\``));
    assert.match(must(box, other, ["artifact", "open", artifact]).out, new RegExp(`\\(${artifact}\\) resolves to that path`));
});

test("30: a context budget that cannot hold the row omits the whole row, never the pointer alone", () =>
{
    const { box, demo, artifact } = withGuide();
    ...
    {
        must(box, demo, ["state", "add", `filler ${index} `.repeat(40), "--exposure", "full", "--priority", "1"]);
    }
    const text = "API design follows the guide";
    must(box, demo, ["state", "add", text, "--exposure", "full", "--priority", "900", "--artifact", artifact]);
    const rendered = must(box, demo, ["context"]).out;
...
test("31: the handoff packet carries the pointer", () =>
{
    const { box, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]);
    const work = workIdIn(must(box, demo, ["work", "add", "the flow works"]).out);
...
test("32: the terminal render carries the pointer", () =>
{
    const { box, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]);
    assert.match(must(box, demo, ["context", "--pretty"]).out,
        new RegExp(`API design follows the guide — see \`self artifact open ${artifact}\``));
});

test("33: `self search --type convention` carries the pointer", () =>
{
    const { box, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]);
    assert.match(must(box, demo, ["search", "--type", "convention", "--all"]).out,
        new RegExp(`API design follows the guide — see \`self artifact open ${artifact}\``));
});

test("34: bytes this machine has not synced leave the pointer standing and the open refused", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]);
    rmSync(storedFile(ws, artifact));
    assert.match(must(box, demo, ["context"]).out, new RegExp(`— see \`self artifact open ${artifact}\``));
    const refused = selfIn(box, demo, ["artifact", "open", artifact]);
    assert.notEqual(refused.code, 0);
    assert.match(refused.out, /run `self sync` to fetch it/);
});

test("35: `self status` says one line about a live record whose artifact is missing", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    const rule = idIn(must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]).out);
    rmSync(storedFile(ws, artifact));
    const health = must(box, demo, ["status"]).out.split("\n").filter((line) => line.includes(artifact));
    assert.equal(health.length, 1);
    assert.match(health[0], new RegExp(`${rule} artifact ${artifact} guide.md is missing from this store`));
});

test("36: an artifact whose stored bytes no longer match its digest raises a signal", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "API design follows the guide", "--artifact", artifact]);
...
test("38: a successor stated without --artifact drops the pointer, and the artifact stays stored", async () =>
{
    ...
    assert.match(must(box, demo, ["artifact", "list"]).out, new RegExp(artifact));
});

test("43: only the project whose log holds the record says the line", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    must(box, demo, ["convention", "add", "every project follows the guide", "--artifact", artifact, "--workspace"]);
    const other = join(ws, "other");
    ...
    assert.equal(must(box, other, ["status"]).out.split("\n").filter((line) => line.includes(artifact)).length, 0);
});

test("44: a proposal is live, so its reference is checked too", () =>
{
    const { box, ws, demo, artifact } = withGuide();
    const proposal = entityIdIn(must(box, demo,
...
test("46: a path input is two writes, so the store takes two commits", () =>
{
    const box = machine();
    const { ws, demo } = demoWorkspace(box);
    writeFileSync(join(demo, "guide.md"), "the guide\n");

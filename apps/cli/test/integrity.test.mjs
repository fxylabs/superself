...
const { demo: mixedDemo } = await demoWorkspace(mixedBox);
...
test("E1 / #305 G3: every captured read surface answers exactly as its capture states", async () =>
{
    ...
    for (const [name, args] of manifest)
    {
        ...
        const result = await must(e1, ws, args);
...
test("#305 G4: `self log` prints the dropped units' events exactly as it did before", async () =>
{
    const expected = readFileSync(join(fixtures, "pre-cutover-reads", "log.txt"), "utf8");
    const printed = (await must(e1, ws, ["log", "-n", "500", "--project", "demo"])).out;
...
test("#305 G5: what a rebaselined capture says about criteria and coverage did not move", async () =>
{
    const objective = (await must(e1, ws, ["objective", "show", "o-yfxat", "--project", "demo"])).out;
    ...
    const status = (await must(e1, ws, ["status", "--project", "demo"])).out;
...
test("#305 G8: a verb handed a pre-cutover work id refuses it as unknown", async () =>
{
    for (const args of [["handoff", "w-2hcs1"], ["work", "show", "w-2hcs1"]])
    {
        const refused = await selfIn(e1, ws, [...args, "--project", "demo"]);
...
test("E1: a pre-cutover store answers search over its live records, not its log", async () =>
{
    const decisions = (await must(e1, ws, ["search", "ship", "--type", "decision", "--all"])).out;
    ...
    const entities = (await must(e1, ws, ["search", "raw", "--type", "entity", "--all"])).out;
    assert.match(entities, /raw standing note/);
    assert.match(entities, /\(proposed\)/, "a proposed raw entity lost its marker");
});

test("#305 G1: the fold rewrites no line of the log it read", async () =>
{
    await refold();
...
test("#305 G2: the page of a unit the fold no longer answers for is removed", async () =>
{
    await refold();
...
test("#305 G7: the hash entry of a removed page goes with it", async () =>
{
    await refold();
...
test("#305 G5 (canonical files): the cutover fold moved only what names work", async () =>
{
    await refold();
...
test("#305 G6: a second fold changes nothing — the sweep has nothing left to take", async () =>
{
    await refold();
...
test("E2: lifecycle verbs target legacy-derived records and record entity events referencing them", async () =>
{
    ...
    await must(mixedBox, mixedDemo, ["fold"]);
    ...
    const confirmed = await must(mixedBox, mixedDemo, ["decide", "confirm", "01hz00000000000000000000e1"]);
    assert.match(confirmed.out, /entity\.confirmed recorded/);
    assert.ok((await must(mixedBox, mixedDemo, ["state", "show", "01hz00000000000000000000e1"])).out.includes("confirmed"));
    assert.ok((await must(mixedBox, mixedDemo, ["context"])).out.includes("adopt the queue"),
    ...
    await must(mixedBox, mixedDemo, ["state", "place", "01hz00000000000000000000e2", "--priority", "5"]);
    assert.ok((await must(mixedBox, mixedDemo, ["state", "show", "01hz00000000000000000000e2"])).out.includes("placement: project · full · priority 5"));
    ...
    assert.ok(!(await must(mixedBox, mixedDemo, ["context"])).out.includes("legacy standing rule"),
    ...
    assert.equal((await must(mixedBox, mixedDemo, ["search", "legacy standing rule"])).out.trim(), "no matches");
    assert.match((await must(mixedBox, mixedDemo, ["state", "show", "01hz00000000000000000000e2", "--history"])).out, /dropped/);
});

test("E3: two folds of a mixed store render identically", async () =>
{
    await must(mixedBox, mixedDemo, ["decide", "a native ruling beside legacy history"]);
    await must(mixedBox, mixedDemo, ["fold"]);
    const context = (await must(mixedBox, mixedDemo, ["context"])).out;
    const list = (await must(mixedBox, mixedDemo, ["state", "list"])).out;
    const stateFile = join(mixedBox.root, "ws", ".superself", "projects", "demo", "state.md");
    const stateBefore = readFileSync(stateFile, "utf8");
    await must(mixedBox, mixedDemo, ["fold"]);
    assert.equal((await must(mixedBox, mixedDemo, ["context"])).out, context, "a refold changed the rendered context");
    assert.equal((await must(mixedBox, mixedDemo, ["state", "list"])).out, list, "a refold changed the entity list");
...
test("E6: one representative verb per family prints the entity event it records", async () =>
{
    ...
    assert.match((await must(box, demo, ["goal", "add", "own the niche"])).out, /entity\.confirmed recorded/);
    const proposed = idIn((await must(box, demo, ["decide", "maybe so", "--proposed"])).out);
    assert.match((await must(box, demo, ["decide", "decline", proposed, "--why", "not now"])).out, /entity\.retracted recorded/);
    ...
    assert.match((await must(box, demo, ["state", "place", raw, "--priority", "3"])).out, /entity\.placed recorded/);
});

...
async function fixture()
{
    ...
    const old = objectiveIdIn(run(["objective", "add", "plan v1", "--target", "2099-01-01", "--success", "s1", "--stop", "st1"]));
    const other = objectiveIdIn(run(["objective", "add", "other objective"]));
    const m1 = milestoneIdIn(run(["milestone", "add", "m1 unstarted", "--objective", old, "--exit", "a"]));
    const m2 = milestoneIdIn(run(["milestone", "add", "m2 on-track", "--objective", old, "--exit", "b"]));
    const w1 = workIdIn(run(["work", "add", "w1 for m2"]));
    run(["work", "link", w1, "--milestone", m2]);
    const m3 = milestoneIdIn(run(["milestone", "add", "m3 partial", "--objective", old, "--exit", "c", "--exit", "d"]));
    const w2 = workIdIn(run(["work", "add", "w2 for m3 and other"]));
    run(["work", "link", w2, "--milestone", m3, "--objective", other]);
    run(["milestone", "met", m3, "--criterion", "c1", "--why", "first half", "--work", w2]);
    const m4 = milestoneIdIn(run(["milestone", "add", "m4 reached", "--objective", old, "--exit", "e"]));
    const w3 = workIdIn(run(["work", "add", "w3 for m4"]));
    run(["work", "link", w3, "--milestone", m4]);
    run(["milestone", "met", m4, "--criterion", "c1", "--why", "done", "--work", w3]);
    run(["milestone", "reach", m4]);
    const m5 = milestoneIdIn(run(["milestone", "add", "m5 dropped", "--objective", old, "--exit", "f"]));
...
test("cell 1: an unstarted milestone with no work and no coverage carries unchanged", () =>
{
    const page = afterPage(F.m1);
...
test("cell 2: an on-track milestone carries with its one linked unit and its target untouched", () =>
{
    const page = afterPage(F.m2);
...
test("cell 3: partial coverage and a unit linked elsewhere both survive the carry", () =>
{
    const page = afterPage(F.m3);
    assert.match(objectiveLine(page), new RegExp(`^${F.successor} `));
    assert.match(page, /c1 — c _\(covered\)_/);
    assert.match(page, /c2 — d _\(open\)_/);
    assert.match(page, new RegExp(`- Work: ${F.w2}$`, "m"));
    assert.match(F.run(["work", "show", F.w2]), new RegExp(`Contributes to: ${F.other} other objective \\(on-track\\); ${F.m3} m3 partial \\(on-track\\)`));
});

test("cell 4: a reached milestone carries with its reach, criteria and evidence intact", () =>
{
    const page = afterPage(F.m4);
    assert.match(objectiveLine(page), new RegExp(`^${F.successor} `));
...
test("cell 5: a dropped milestone stays under the predecessor and is not counted", () =>
{
    const page = afterPage(F.m5);
    assert.match(objectiveLine(page), new RegExp(`^${F.old} plan v1`));
    assert.match(page, /^- State: closed — dropped — not needed$/m);
    assert.ok(!F.run(["objective", "show", F.successor]).includes(`## Milestone ${F.m5}`), "the dropped milestone moved");
    assert.equal(carryLinks(F.ws, F.successor).some((event) => event.payload.entity === F.m5), false);
});

test("cell 6: a milestone already superseded stays behind and its successor carries", () =>
{
    const stayed = afterPage(F.ma);
    assert.match(objectiveLine(stayed), new RegExp(`^${F.old} `));
    assert.match(stayed, new RegExp(`^- State: closed — superseded by ${F.mb} — `, "m"));
    const moved = afterPage(F.mb);
    assert.match(objectiveLine(moved), new RegExp(`^${F.successor} `));
    assert.match(moved, /^- State: unstarted — /m);
});

test("cell 7: the receipt counts the five carried, and the predecessor keeps only the dropped and the superseded one", () =>
{
    assert.match(F.receipt, new RegExp(`^${F.successor} — carried 5 milestones from ${F.old}$`, "m"));
    const page = F.run(["objective", "show", F.old]);
...
{
    const bare = objectiveIdIn(F.run(["objective", "add", "bare objective"]));
...
test("cell 9: the predecessor's page lists the carried milestones and never says its objective is closed", () =>
{
    const page = F.run(["objective", "show", F.old]);
...
test("cell 10: the successor's page carries the lineage, the progress of the carried milestones, and their states as they were", () =>
{
    const page = F.run(["objective", "show", F.successor]);
...
test("cell 11: a carried milestone's page names the successor as its objective and says where it came from", () =>
{
    const page = afterPage(F.m3);
...
test("cell 12: self context lists the successor and no longer the superseded objective", () =>
{
    const out = F.run(["context"]);
    assert.match(out, /\[objective\] plan v2/);
    assert.ok(!out.includes("[objective] plan v1"), out);
});

test("cell 13: a unit linked only through a milestone keeps its contribution line", () =>
{
    assert.match(F.run(["work", "show", F.w1]), new RegExp(`^- Contributes to: ${F.m2} m2 on-track \\(on-track\\)$`, "m"));
});

test("cell 14: the objective listing shows the successor with its milestones and not the predecessor", () =>
{
    const out = F.run(["objective"]);
...
test("cell 20: coverage recorded after the carry lands on the milestone and counts on the successor", () =>
{
    F.run(["milestone", "met", F.m1, "--criterion", "c1", "--why", "judged after the move"]);
    assert.match(afterPage(F.m1), /c1 — a _\(covered\)_/);
    assert.match(F.run(["objective", "show", F.successor]), /^- Progress: 3 of 7 exit criteria covered$/m);
});
...
test("cell 18: milestone drop is the explicit way a carried milestone leaves, and the carry stays on record", async () =>
{
    ...
    assert.match(F.run(["objective", "show", F.old]), new RegExp(`^- Milestones carried to ${F.successor}: .*${F.m2}`, "m"));
});
...
test("cell 19: milestone revise after the carry creates the successor milestone under the successor objective only", async () =>
{
    ...
    assert.match(afterPage(F.m3), new RegExp(`^- State: closed — superseded by ${next} — `, "m"));
    assert.match(afterPage(next), new RegExp(`^- Objective: ${F.successor} `, "m"));
});

test("cell 21: objective revise from a directory outside every registered project is refused and records nothing", () =>
{
    const size = eventsOf(F.ws).length;
    const result = selfIn(F.box, F.box.root, ["objective", "revise", F.successor, "--why", "from nowhere", "--outcome", "x"]);
    assert.notEqual(result.code, 0);
    assert.match(result.out, /not inside a registered project/);
    assert.equal(eventsOf(F.ws).length, size);
});

test("cell 22: objective show --project from the workspace root renders the carried line", () =>
{
    const page = must(F.box, F.ws, ["objective", "show", F.old, "--project", "demo"]).out;
    assert.match(page, new RegExp(`^- Milestones carried to ${F.successor}: `, "m"));
    assert.ok(!page.includes("its objective is closed"), page);
});

test("cell 23: milestone show --project from the workspace root names the successor and the source", () =>
{
    const page = must(F.box, F.ws, ["milestone", "show", F.m1, "--project", "demo"]).out;
    assert.match(page, new RegExp(`^- Objective: ${F.successor} `, "m"));
    assert.match(page, new RegExp(`^- Carried from: ${F.old}$`, "m"));
});

test("cell 24: a revise the supersede gate refuses writes neither the successor nor a single carry link", () =>
{
    const gated = objectiveIdIn(F.run(["objective", "add", "gated objective"]));
    F.run(["milestone", "add", "gated checkpoint", "--objective", gated, "--exit", "one"]);
    const size = eventsOf(F.ws).length;
    const result = selfIn(F.box, F.demo, ["objective", "revise", gated, "--why", "piped", "--outcome", "gated, reworded"]);
    assert.notEqual(result.code, 0);
...
test("cell 16: undoing one carry link returns that milestone alone to the predecessor", async () =>
{
    ...
    F.run(["undo", link.id, "--why", "this one was never part of the revised plan"]);
    const page = afterPage(F.m1);
    ...
    assert.ok(!F.run(["objective", "show", F.successor]).includes(`## Milestone ${F.m1}`));
    assert.match(F.run(["objective", "show", F.old]), new RegExp(`^- Milestones carried to ${F.successor}: ${F.m2}, ${F.m3}, ${F.m4}, ${F.mb}$`, "m"));
});
...
test("cell 17: a second revision carries the milestones again, one hop per page", async () =>
{
    ...
    const newest = G.run(["objective", "show", newer]);
    for (const id of ids)
    {
        assert.ok(newest.includes(`## Milestone ${id}`), `${id} did not reach the newest objective`);
    }
    assert.match(G.run(["objective", "show", G.successor]), new RegExp(`^- Milestones carried to ${newer}: ${ids.join(", ")}$`, "m"));
    assert.match(G.run(["objective", "show", G.old]), new RegExp(`^- Milestones carried to ${G.successor}: ${ids.join(", ")}$`, "m"));
    assert.match(G.run(["milestone", "show", G.m1]), new RegExp(`^- Carried from: ${G.old}, ${G.successor}$`, "m"));
});
...
test("cells 25–28: whatever the revise changes, the milestone carries and the successor shows the change", async () =>
{
    ...
    {
        const old = objectiveIdIn(run(["objective", "add", `objective ${cell}`, "--target", "2099-01-01"]));
        const milestone = milestoneIdIn(run(["milestone", "add", `checkpoint ${cell}`, "--objective", old, "--exit", "one"]));
        const work = workIdIn(run(["work", "add", `unit ${cell}`]));
        run(["work", "link", work, "--milestone", milestone]);
        ...
        const page = run(["objective", "show", successor]);
        shows.forEach((pattern) => assert.match(page, pattern, `cell ${cell}: ${pattern}`));
        const shown = run(["milestone", "show", milestone]);
...
test("cell 30: a blocked milestone is live and carries with its blocked state", async () =>
{
    ...
    const run = (args) => must(box, demo, args).out;
    const old = objectiveIdIn(run(["objective", "add", "blocked plan"]));
    const milestone = milestoneIdIn(run(["milestone", "add", "blocked checkpoint", "--objective", old, "--exit", "one"]));
    const work = workIdIn(run(["work", "add", "stuck unit"]));
    run(["work", "link", work, "--milestone", milestone]);
    run(["work", "block", work, "--on", "external", "--why", "vendor outage"]);
    assert.match(run(["milestone", "show", milestone]), /^- State: blocked — /m);
    ...
    const page = run(["milestone", "show", milestone]);
...
test("cell 29: a legacy milestone carries to the successor like a native one", async () =>
{
    ...
    assert.match(run(["milestone", "show", "m-lega1"]), /^- Objective: o-lega1 legacy plan$/m);
    ...
    const page = run(["milestone", "show", "m-lega1"]);

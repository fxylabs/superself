...
async function workspace()
{
    ...
    await must(box, alpha, ["goal", "add", "a direction"]);
    const objective = (await must(box, alpha, ["objective", "add", "a measurable outcome", "--target", "2099-01-01"]))
        .out.match(/\bo-[0-9a-z]{5}\b/)[0];
    return { box, ws, alpha, beta, objective };
}

const { box, ws, alpha, beta, objective } = await workspace();
...
async function decisionIn(cwd, text)
{
    return idIn((await must(box, cwd, ["decide", text, "--why", "it was weighed", "--proposed"])).out);
}

async function recordIn(cwd, text)
{
    return entityIn((await must(box, cwd, ["state", "add", text, "--proposed"])).out);
}

async function proposalIn(cwd, outcome)
{
    const out = (await must(box, cwd, ["work", "propose", outcome, "--objective", objective,
...
async function capped()
{
    ...
    await must(capBox, other, ["project", "init", "--name", "beta", "--no-connect"]);
...
async function waitingRecords(render)
{
    await proposalIn(alpha, `an outcome advertised to a ${render} reader outside the project`);
    await must(box, alpha, ["decide", `a direction advertised to a ${render} reader outside`, "--why", "it was weighed", "--proposed"]);
    await must(box, alpha, ["state", "add", `a record advertised to a ${render} reader outside`, "--proposed"]);
}
...
VERBS.forEach((verb, index) =>
{
    for (const place of PLACES)
    {
        ...
        test(`cell ${number}: ${verb.name} run from ${place.name} records into the owning project`, async () =>
        {
...
test("cells 13-16 setup: the owning project's checkout is removed after its records exist", async () =>
{
    ...
    const target = (await must(goneBox, dir, ["objective", "add", "a measurable outcome", "--target", "2099-01-01"]))
        .out.match(/\bo-[0-9a-z]{5}\b/)[0];
    const propose = async (outcome) => workIdIn((await must(goneBox, dir, ["work", "propose", outcome, "--objective", target,
        "--value", BRIEF.value, "--success", BRIEF.success, "--stop", BRIEF.stop,
        "--risk", BRIEF.risk, "--capacity", BRIEF.capacity, "--evidence-plan", BRIEF.evidencePlan,
        "--confidence", BRIEF.confidence, "--expires", BRIEF.expires])).out);
...
test("cell 17: from the workspace root, an id no project holds is refused by naming what was searched", async () =>
{
    for (const verb of VERBS)
    {
        const refused = await selfIn(box, ws, verb.answer("e-zzzzz"));
...
test("cell 18: inside a project, an id it does not hold keeps the verb's own refusal", async () =>
{
    ...
    {
        const refused = await selfIn(box, alpha, verb.answer("e-zzzzz"));
        assert.notEqual(refused.code, 0, `${verb.name} answered for an id nothing holds`);
        assert.match(refused.out, refusals[index]);
    }
});

test("cell 19: a prefix two projects both answer to is refused by naming both", async () =>
{
    const here = idIn((await must(box, alpha, ["decide", "a direction chosen in alpha", "--why", "it was weighed", "--proposed"])).out);
    const there = idIn((await must(box, beta, ["decide", "a direction chosen in beta", "--why", "it was weighed", "--proposed"])).out);
    const prefix = sharedPrefix(here, there);
    assert.ok(prefix.length > 0, "the two decision ids share no prefix to be ambiguous about");
    const refused = await selfIn(box, ws, ["decide", "confirm", prefix]);
...
test("cells 20 and 21: an archived owner is refused by the append gate, wherever the call is made", async () =>
{
    ...
    await must(archivedBox, archivedWs, ["project", "archive", "alpha", "--why", "nobody is working on it"]);
    const expected = /project "alpha" is archived, so nothing more is recorded into it/;
    const inside = await selfIn(archivedBox, dir, ["state", "confirm", id]);
    assert.notEqual(inside.code, 0, "a confirm landed in an archived project");
    assert.match(inside.out, expected);
    const outside = await selfIn(archivedBox, archivedWs, ["state", "confirm", id]);
...
test("cell 22: with no workspace on this machine, every one of the four says so", async () =>
{
    const bare = machine();
    const nowhere = mkdtempSync(join(tmpdir(), "self-no-workspace-"));
    for (const verb of VERBS)
    {
        const refused = await selfIn(bare, nowhere, verb.answer("e-zzzzz"));
...
test("cell 23: from the workspace root, the room judgment is made against the owning project", async () =>
{
    const { capBox, capWs, owner } = await capped();
    setCaps(capBox, { indexTokens: 24 });
    await must(capBox, owner, ["state", "add", "holds the one index seat"]);
    const added = entityIn((await must(capBox, owner, ["state", "add", "wants the seat", "--proposed"])).out);
    const refused = await selfIn(capBox, capWs, ["state", "confirm", added]);
...
test("cell 24: from the workspace root, a cap-paired demotion still lands as one unit", async () =>
{
    const { capBox, capWs, owner } = await capped();
    setCaps(capBox, { indexTokens: 24 });
    const holder = entityIn((await must(capBox, owner, ["state", "add", "holds the one index seat"])).out);
    const added = entityIn((await must(capBox, owner, ["state", "add", "wants the seat", "--proposed", "--demote", holder])).out);
    const swap = await must(capBox, capWs, ["state", "confirm", added]);
    assert.equal([...swap.out.matchAll(/entity\.confirmed/g)].length, 2, `the pair did not land as one unit:\n${swap.out}`);
    assert.ok((await must(capBox, capWs, ["state", "show", added, "--project", "alpha"])).out.includes("placement: project · index"));
});
...
test("cell 25: every line self context --project prints runs where it was read", async () =>
{
    await waitingRecords("context");
...
test("cell 27: none of the four gained a scope flag", async () =>
{
    for (const verb of VERBS)
    {
        const refused = await selfIn(box, ws, [...verb.answer("e-zzzzz"), "--project", "alpha"]);

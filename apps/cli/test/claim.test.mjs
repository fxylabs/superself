...
{
    seq += 1;
    return entityIdIn((await must(box, demo, ["state", "add", `raw record ${seq}`])).out);
}
...
test("an unclaimed unit is claimed, and the brief comes back with it", async () =>
{
    const id = await freshUnit();
    const result = await selfIn(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
...
test("the holding session gets the brief again and records no second claim", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
    const again = await selfIn(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
...
test("a live holder is disclosed, the brief still comes back, and nothing is refused", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
...
test("a holder this machine never recorded reads as last recorded, not as a place", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "start", id], asSession("alpha"));
    const other = await selfIn(box, demo, ["work", "start", id], asSession("beta", ALIVE));
...
test("a holder whose process is gone reads as ended, and the claim moves", async () =>
{
    const id = await freshUnit();
...
test("a done unit is refused and records no claim", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "done", id, "--report", "shipped in commit abc123"], asSession("alpha", ALIVE));
    const before = startsOf(id).length;
    const result = await selfIn(box, demo, ["work", "start", id], asSession("beta", ALIVE));
...
test("a non-holder is told who holds the unit and is not refused", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
    const reported = await selfIn(box, demo, ["report", id, "progress from another session"], asSession("beta", ALIVE));
    assert.equal(reported.code, 0, "a non-holder may report");
    assert.match(reported.out, /held by another session, running since/);
    const done = await selfIn(box, demo, ["work", "done", id, "--report", "closed in commit def456"], asSession("beta", ALIVE));
...
test("a start with no session recorded reads as unclaimed, not as an anonymous holder", async () =>
{
    const id = await freshUnit();
...
test("work show reads three times and records nothing", async () =>
{
    const id = await freshUnit();
    const before = readFileSync(log, "utf8").length;
    await must(box, demo, ["work", "show", id], asSession("alpha", ALIVE));
    await must(box, demo, ["work", "show", id], asSession("beta", ALIVE));
    await must(box, demo, ["work", "show", id], asSession("beta", ALIVE));
    assert.equal(readFileSync(log, "utf8").length, before, "looking at a unit is never a claim");
});

test("work show renders the holder above the brief", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
    const shown = (await must(box, demo, ["work", "show", id], asSession("beta", ALIVE))).out;
...
test("the holder renders in context and in the work list", async () =>
{
    const id = await freshUnit();
    await must(box, demo, ["work", "start", id], asSession("alpha", ALIVE));
    assert.match((await must(box, demo, ["context"], asSession("beta", ALIVE))).out, /held by another session, running since/);
    assert.match((await must(box, demo, ["work"], asSession("beta", ALIVE))).out, /held by another session, running since/);
    assert.match((await must(box, demo, ["work"], asSession("alpha", ALIVE))).out, /held by this session/);
});

test("state start: cells 1 and 6 — an unclaimed record is claimed", async () =>
{
    const open = await freshEntity();
    const first = await must(box, demo, ["state", "start", open], asSession("alpha", ALIVE));
    assert.doesNotMatch(first.out, /held by/, "cell 1: nothing held it");
    assert.equal(entityStartsOf(open).length, 1);

    const legacy = await freshEntity();
    ...
    const taken = await must(box, demo, ["state", "start", legacy], asSession("alpha", ALIVE));
...
test("state start: cell 2 — the holding session is told, and records no second claim", async () =>
{
    const id = await freshEntity();
    await must(box, demo, ["state", "start", id], asSession("alpha", ALIVE));
    const again = await must(box, demo, ["state", "start", id], asSession("alpha", ALIVE));
    assert.match(again.out, /held by this session/);
    assert.equal(entityStartsOf(id).length, 1);
});

test("state start: cell 3 — a live holder is disclosed and not refused", async () =>
{
    const id = await freshEntity();
    await must(box, demo, ["state", "start", id], asSession("alpha", ALIVE));
    const other = await selfIn(box, demo, ["state", "start", id], asSession("beta", ALIVE));
...
test("state start: cell 4 — a holder this machine cannot judge reads as last recorded", async () =>
{
    const id = await freshEntity();
    await must(box, demo, ["state", "start", id], asSession("alpha"));
    const other = await selfIn(box, demo, ["state", "start", id], asSession("beta", ALIVE));
    assert.equal(other.code, 0);
...
test("state start: cell 5 — a holder whose process is gone hands the claim over", async () =>
{
    const id = await freshEntity();
    await must(box, demo, ["state", "start", id], asSession("alpha", GONE));
    const other = await must(box, demo, ["state", "start", id], asSession("beta", ALIVE));
...
{
    const blocked = await freshEntity();
    await must(box, demo, ["state", "start", blocked], asSession("alpha", ALIVE));
    await must(box, demo, ["state", "block", blocked, "--why", "waiting on upstream"], asSession("alpha", ALIVE));
    const onBlocked = await selfIn(box, demo, ["state", "start", blocked], asSession("beta", ALIVE));
    assert.equal(onBlocked.code, 1, "cell 7");
    assert.match(onBlocked.out, /is blocked — unblock it first/);

    const done = await freshEntity();
    await must(box, demo, ["state", "done", done, "--report", "verified output landed"], asSession("alpha", ALIVE));
    const onDone = await selfIn(box, demo, ["state", "start", done], asSession("beta", ALIVE));
...
test("state start: cell 9 — a work record keeps its own verb, and the judgment is still shared", async () =>
{
    const unit = await freshUnit();
    await must(box, demo, ["work", "start", unit], asSession("alpha", ALIVE));
    const viaState = await selfIn(box, demo, ["state", "start", unit], asSession("beta", ALIVE));

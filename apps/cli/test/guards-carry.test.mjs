...
test("22: the retarget the refusal printed is recorded and moves the plan", async () =>
{
    ...
    assert.equal(revision.payload.gap.milestone, drifted.successor);
});
...
test("38: a coverage claim records the objective it was judged under", () =>
{
    ...
    assert.equal(claim.payload.judgedUnder, settled.objective);
});
...
test("46: coverage recorded after the carry names the parent it was made under", async () =>
{
    ...
    assert.equal(claim.payload.judgedUnder, successor);
...
test("47: undoing the recheck brings the condition back", async () =>
{
    ...
    const recheck = eventsOf(F.ws).find((event) => event.type === "entity.covered"
        && event.payload.judgedUnder === F.successor);
...
test("59: a coverage claim's judged-under parent reads in the checkpoint's own history, not the objective's", async () =>
{
    const milestoneHistory = await settled.run(["state", "show", settled.milestone, "--history"]);
    assert.match(milestoneHistory, /the first half landed/);
    const objectiveHistory = await settled.run(["state", "show", settled.objective, "--history"]);
    assert.ok(!objectiveHistory.includes("the first half landed"),
        `the objective's own history carries a claim judged under it: ${objectiveHistory}`);
});

test("60: a retarget's gap reads in the plan's own history, not the destination's, for --milestone and --objective alike", async () =>
{
    // The --milestone alternative: the retarget cell 22 already recorded.
    const milestoneWork = await drifted.run(["state", "show", drifted.proposal, "--history"]);
    assert.match(milestoneWork, /closed before this was accepted/);
    const milestoneDestination = await drifted.run(["state", "show", drifted.successor, "--history"]);
    assert.ok(!milestoneDestination.includes("closed before this was accepted"),
        `the destination milestone's own history carries the plan's retarget: ${milestoneDestination}`);

    // The --objective alternative: the retarget the `moved` fixture recorded.
    const objectiveWork = await moved.run(["state", "show", moved.proposal, "--history"]);
    assert.match(objectiveWork, /the gap moved/);
    const objectiveDestination = await moved.run(["state", "show", moved.other, "--history"]);
    assert.ok(!objectiveDestination.includes("the gap moved"),
        `the destination objective's own history carries the plan's retarget: ${objectiveDestination}`);
});

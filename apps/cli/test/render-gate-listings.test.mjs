...
must(box, demo, ["project", "init", "--name", "demo", "--desc", "the listing migration", "--no-connect"]);
must(box, other, ["project", "init", "--name", "other", "--desc", "the second log", "--no-connect"]);

const objective = idOf(must(box, demo, ["objective", "add", "every listing states its size"]).out, "o");
must(box, demo, ["objective", "add", "the gate owns the size line"]);
const milestone = idOf(must(box, demo, ["milestone", "add", "the listings answer with blocks",
    "--objective", objective, "--exit", "no listing prints for itself"]).out, "m");
const open = workIdIn(must(box, demo, ["work", "add", "the listings move behind the gate"]).out);
must(box, demo, ["work", "link", open, "--milestone", milestone]);
const second = workIdIn(must(box, demo, ["work", "add", "the size line is written once"]).out);
const finished = workIdIn(must(box, demo, ["work", "add", "the receipts moved in stage two"]).out);
must(box, demo, ["work", "done", finished, "--report", "stage two shipped"]);
writeFileSync(join(demo, "evidence.md"), "a listing carries an artifact\n");
must(box, demo, ["report", open, "the listing sweep", "--artifact", "evidence.md"]);
must(box, other, ["work", "add", "the other project keeps its own log"]);
...
test("stage 3 cell 1: a piped `self project` ends with the number of projects", () =>
{
    const printed = lines(selfIn(box, ws, ["project"]));
...
test("stage 3 cell 2: a piped `self project --archived` counts the projects, not the ways back", () =>
{
    const archiveBox = archivedWorkspace();
    const printed = lines(selfIn(archiveBox.box, archiveBox.ws, ["project", "--archived"]));
...
test("stage 3 cell 3: a piped `self project archive` answers with its receipt, the listing, and the size", () =>
{
    const archiveBox = machineWithProjects(["alpha"]);
    const printed = lines(selfIn(archiveBox.box, archiveBox.ws,
...
test("stage 3 cell 3: a piped `self project restore` answers the same way, with what is still set aside", () =>
{
    const archiveBox = machineWithProjects(["alpha", "beta"]);
    must(archiveBox.box, archiveBox.ws, ["project", "archive", "alpha", "--why", "set aside"]);
    must(archiveBox.box, archiveBox.ws, ["project", "archive", "beta", "--why", "set aside too"]);
    const printed = lines(selfIn(archiveBox.box, archiveBox.ws, ["project", "restore", "alpha"]));
...
test("stage 3 cell 4: a piped `self objective` counts objectives, not the checkpoints under them", () =>
{
    const printed = lines(selfIn(box, demo, ["objective"]));
...
test("stage 3 cell 5: a piped `self milestone` ends with the number of checkpoints", () =>
{
    const printed = lines(selfIn(box, demo, ["milestone"]));
    assert.equal(printed.length, 2);
    assert.equal(printed.at(-1), "1 milestone");
});

test("stage 3 cell 6: a piped `self work` counts the open units and keeps the hidden buckets", () =>
{
    const printed = lines(selfIn(box, demo, ["work"]));
    assert.equal(printed.length, 4);
...
test("stage 3 cell 6: a retired unit stays its own bucket line and is not counted as open", () =>
{
    const retireBox = machineWithProjects([]);
    const unit = workIdIn(must(retireBox.box, retireBox.demo, ["work", "add", "an outcome that moved"]).out);
    must(retireBox.box, retireBox.demo, ["work", "add", "the outcome it moved to"]);
    ...
    const printed = lines(selfIn(retireBox.box, retireBox.demo, ["work"]));
    assert.equal(printed.at(-2), "(1 retired — see log)");
    assert.equal(printed.at(-1), "1 open work unit");
});

test("stage 3 cell 9: a piped `self artifact list` ends with the number of artifacts", () =>
{
    const printed = lines(selfIn(box, demo, ["artifact", "list"]));
    assert.equal(printed.length, 2);
    assert.equal(printed.at(-1), "1 artifact");
});

test("stage 3 cell 9: `self artifact search` counts its hits the same way", () =>
{
    const printed = lines(selfIn(box, demo, ["artifact", "search", "evidence"]));
    assert.equal(printed.at(-1), "1 artifact");
});

test("stage 3 cell 10: a piped `self search` ends with the number of matches", () =>
{
    const printed = lines(selfIn(box, demo, ["search", "listing", "--all"]));
    const hits = printed.length - 1;
    assert.ok(hits >= 2, printed.join("\n"));
    assert.equal(printed.at(-1), `${hits} matches`);
});

test("stage 3 cell 10: one match is counted in the singular the noun actually takes", () =>
{
    const printed = lines(selfIn(box, demo, ["search", "the size line is written once", "--all"]));
    assert.equal(printed.length, 2);
    assert.equal(printed.at(-1), "1 match");
});

test("stage 3 cell 11: a piped `self alias` ends with the number of rows in the table", () =>
{
    const printed = lines(selfIn(box, demo, ["alias"]));
    ...
    must(box, demo, ["alias", "add", "note", "--label", "note", "--exposure", "search"]);
    const after = lines(selfIn(box, demo, ["alias"]));
    assert.equal(after.at(-1), `${printed.length} aliases`);
});

/* ── the truncated listing: the only one that says what it is hiding ─ */

test("stage 3 cell 7: a piped `self log` past its limit says what it shows of what there is", () =>
{
    const total = lines(selfIn(box, demo, ["log", "-n", "500"])).length - 1;
    const printed = lines(selfIn(box, demo, ["log", "-n", "3"]));
    assert.equal(printed.length, 4);
    assert.equal(printed.at(-1), `last 3 of ${total} events · self log -n ${total} --project 'demo'`);
});

test("stage 3 cell 7: a `self log` that shows the whole log states its size and points nowhere", () =>
{
    const total = lines(selfIn(box, demo, ["log", "-n", "500"])).length - 1;
    assert.equal(lines(selfIn(box, demo, ["log", "-n", "500"])).at(-1), `${total} events`);
});

test("stage 3 cell 8: a piped `self log --workspace` points at the workspace, not at one project", () =>
{
    const whole = lines(selfIn(box, demo, ["log", "--workspace", "-n", "500"]));
    const total = whole.length - 1;
    const printed = lines(selfIn(box, demo, ["log", "--workspace", "-n", "3"]));
    assert.equal(printed.length, 4);
...
test("stage 3 cell 8: the total is the merged log's, not the project's the command ran in", () =>
{
    const merged = lines(selfIn(box, demo, ["log", "--workspace", "-n", "500"])).length - 1;
    const here = lines(selfIn(box, demo, ["log", "-n", "500"])).length - 1;
    assert.ok(merged > here, `${merged} merged vs ${here} local`);
    assert.equal(lines(selfIn(box, demo, ["log", "--workspace", "-n", "1"])).at(-1),
...
test("stage 3 cell 12: an empty listing prints its own wording and nothing under it", () =>
{
    const empty = machineWithProjects([]);
    for (const [args, wording] of EMPTY_IN_PROJECT)
    {
        const answer = selfIn(empty.box, empty.demo, args);
        assert.equal(answer.code, 0, answer.out);
        assert.equal(answer.out, `${wording}\n`, `self ${args.join(" ")}`);
    }
});

test("stage 3 cell 12: a workspace with no project registered answers as it always did", () =>
{
    const bare = machine();
    must(bare, bare.root, ["init"]);
    const answer = selfIn(bare, bare.root, ["project"]);
    assert.equal(answer.code, 0, answer.out);
    assert.equal(answer.out, "no projects registered — run `self project init` inside a project directory\n");
});

test("stage 3 cell 12: a workspace whose every project is archived answers as it always did", () =>
{
    const empty = machineWithProjects([]);
    must(empty.box, empty.ws, ["project", "archive", "demo", "--why", "set aside"]);
    const answer = selfIn(empty.box, empty.ws, ["project"]);
    assert.equal(answer.code, 0, answer.out);
...
test("stage 3 cell 14: from outside every checkout, the size line's pointer names the project asked about", () =>
{
    const total = lines(selfIn(box, demo, ["log", "-n", "500"])).length - 1;
    const printed = lines(selfIn(box, box.root, ["log", "-n", "2", "--project", "demo"]));
    ...
    const followed = lines(selfIn(box, box.root, ["log", "-n", String(total), "--project", "demo"]));
    assert.equal(followed.length, total + 1);
    assert.equal(followed.at(-1), `${total} events`);
});

test("stage 3 cell 14: a scoped listing with no window states the size of the project it was asked about", () =>
{
    assert.equal(lines(selfIn(box, box.root, ["objective", "--project", "demo"])).at(-1), "2 open objectives");
    assert.equal(lines(selfIn(box, box.root, ["work", "--project", "other"])).at(-1), "1 open work unit");
});
...
function machineWithProjects(slugs)
{
    const own = machine();
    ...
    must(own, project, ["project", "init", "--name", "demo", "--desc", "the scratch project", "--no-connect"]);
    for (const slug of slugs)
    {
        const dir = join(root, slug);
        mkdirSync(dir, { recursive: true });
        git(own, dir, ["init", "-q", "-b", "main"]);
        must(own, dir, ["project", "init", "--name", slug, "--no-connect"]);
    }
...
{
    const own = machineWithProjects(["alpha", "beta"]);
    must(own.box, own.ws, ["project", "archive", "alpha", "--why", "set aside for the quarter"]);
    must(own.box, own.ws, ["project", "archive", "beta", "--why", "set aside as well"]);
    return own;
}

...
async function syncedWorkspace()
{
    ...
    const synced = await demoWorkspace(remoteBox);
    const remote = join(remoteBox.root, "remote.git");
    execFileSync("git", ["init", "--bare", "-q", remote], { env: remoteBox.env });
    await must(remoteBox, synced.ws, ["remote", "add", remote]);
    await must(remoteBox, synced.ws, ["sync"]);
...
test("cell 1: bytes the store does not hold are copied to a path of their own, digested from the copy", async () =>
{
    const body = "cell 1 unique bytes\n";
    const { work, code, out } = await attach(["--artifact", fileAt("one.md", body)]);
...
test("cell 2: the same bytes attached again take a second id and share the first's stored path", async () =>
{
    const body = "cell 2 shared bytes\n";
    const first = await attach(["--artifact", fileAt("first.md", body)]);
    assert.equal(first.code, 0, first.out);
    const before = storedNames();
    const second = await attach(["--artifact", fileAt("second.md", body)]);
...
test("cell 3: a record whose stored file this machine does not have is not reused", async () =>
{
...
test("cell 4: a record whose stored bytes no longer match its digest is not reused", async () =>
{
...
test("cell 5: the same bytes in another project are not reused across the project boundary", async () =>
{
    const body = "cell 5 cross-project bytes\n";
    const other = join(ws, "other");
    mkdirSync(other, { recursive: true });
    git(box, other, ["init", "-q", "-b", "main"]);
    await must(box, other, ["project", "init", "--name", "other", "--desc", "a second project"]);
    const first = await attach(["--artifact", fileAt("first.md", body)]);
    const second = await attach(["--artifact", fileAt("second.md", body)], other);
    assert.equal(second.code, 0, second.out);
    const one = attached(first.work)[0];
...
test("cell 6: a directory whose manifest hashes the same shares the stored bundle", async () =>
{
    const entries = { "index.html": "<h1>cell 6</h1>", "assets/app.js": "// six" };
    const first = await attach(["--artifact", tree("dist", entries)]);
    assert.equal(first.code, 0, first.out);
    const before = storedNames();
    const second = await attach(["--artifact", tree("dist", entries)]);
    assert.equal(second.code, 0, second.out);
    const one = attached(first.work)[0];
    const two = attached(second.work)[0];
...
test("cell 7: a directory differing in one member is copied whole", async () =>
{
    const first = await attach(["--artifact", tree("dist", { "index.html": "<h1>cell 7</h1>", "a.txt": "seven" })]);
    const second = await attach(["--artifact", tree("dist", { "index.html": "<h1>cell 7</h1>", "a.txt": "seven and a half" })]);
    assert.equal(second.code, 0, second.out);
    const one = attached(first.work)[0];
    const two = attached(second.work)[0];
...
test("cell 8: a stored bundle missing one member is not reused", async () =>
{
...
test("cell 9: a pruned record is out of the index, so the same bytes are stored again", async () =>
{
    ...
    const first = await attach(["--artifact", fileAt("cell9-first.md", body)]);
    const one = attached(first.work)[0];
    await must(box, demo, ["work", "done", first.work]);
    ...
    const second = await attach(["--artifact", fileAt("cell9-second.md", body)]);
    assert.equal(second.code, 0, second.out);
    const two = attached(second.work)[0];
...
test("cell 10: one source declared twice in one report is still refused before anything is stored", async () =>
{
    const path = fileAt("twice.md", "cell 10 bytes\n");
    const before = storedNames();
    const { work, code, out } = await attach(["--artifact", path, "--artifact", path]);
...
test("cell 11: two files of one report holding the same bytes are stored once and get an id each", async () =>
{
    const body = "cell 11 twin bytes\n";
    const before = storedNames();
    const { work, code, out } = await attach(["--artifact", fileAt("left.md", body), "--artifact", fileAt("right.md", body)]);
    assert.equal(code, 0, out);
...
test("cell 12: a report that fails after reusing bytes leaves the bytes it reused alone", async () =>
{
    ...
    const failed = await attach(["--artifact", fileAt("second.md", body), "--artifact", fileAt("new.md", "cell 12 fresh bytes\n")]);
...
{
    const body = "# cell 13\n\nthe design body\n";
    await attach(["--artifact", fileAt("earlier.md", body)]);
    const decision = idIn((await must(box, demo, ["decide", "cell 13: the store dedupes"])).out);
    const work = await unit();
    const submitted = await must(box, demo, ["report", work, "cell 13 design", "--design",
...
test("cell 14: identical files under a differently named directory do not share, because the index differs", async () =>
{
    const entries = { "a.txt": "cell 14", "b.txt": "cell 14 too" };
    const first = await attach(["--artifact", tree("left", entries)]);
    const second = await attach(["--artifact", tree("right", entries)]);
    assert.equal(second.code, 0, second.out);
    const one = attached(first.work)[0];
    const two = attached(second.work)[0];
...
test("cell 15: identical files under the same directory name share, generated index included", async () =>
{
    const entries = { "a.txt": "cell 15", "b.txt": "cell 15 too" };
    const first = await attach(["--artifact", tree("report", entries)]);
    const before = storedNames();
    const second = await attach(["--artifact", tree("report", entries)]);
    assert.equal(second.code, 0, second.out);
    const one = attached(first.work)[0];
    const two = attached(second.work)[0];
...
test("cell 16: the same members under two entries share the path and keep an entry each", async () =>
{
    const entries = { "index.html": "<h1>cell 16</h1>", "other.html": "<h1>other</h1>" };
    const first = await attach(["--artifact", tree("site", entries)]);
    const second = await attach(["--artifact", tree("site", entries), "--entry", "other.html"]);
    assert.equal(second.code, 0, second.out);
    const one = attached(first.work)[0];
    const two = attached(second.work)[0];
...
test("cell 17: a single file over the byte bound is refused, and stages nothing", async () =>
{
    const before = storedNames();
    const { work, code, out } = await attach(["--artifact", sparseFile("huge.bin", MAX_BYTES + 1)]);
    assert.equal(code, 1, out);
...
test("cell 18: a single file over the warning size says so once and is attached", async () =>
{
    const { work, code, out } = await attach(["--artifact", sparseFile("large.bin", WARN_BYTES + 1)]);
    assert.equal(code, 0, out);
...
test("cell 19: a file under the warning size is attached without a word about its size", async () =>
{
    const { work, code, out } = await attach(["--artifact", fileAt("small.md", "cell 19\n")]);
    assert.equal(code, 0, out);
...
test("cell 20: a store holding no artifact reports zero rather than failing", async () =>
{
    const empty = machine();
    const fresh = await demoWorkspace(empty);
    const { code, out } = await selfIn(empty, fresh.demo, ["store", "size"]);
    assert.equal(code, 0, out);
    assert.match(out, /artifacts: 0 recorded, 0 distinct contents, 0 B in 0 stored files/);
    assert.match(out, /orphan bytes: none/);
});

test("cell 28: a file no record names is reported as an orphan, with its bytes, and is left where it is", async () =>
{
    const stray = join(demoArtifacts, "stray-bytes.md");
    const body = "cell 28 unnamed bytes\n";
    writeFileSync(stray, body);
    const { code, out } = await selfIn(box, demo, ["store", "size"]);
...
test("cell 29: a store whose every stored file is named reports no orphan", async () =>
{
    const { code, out } = await selfIn(box, demo, ["store", "size"]);
    assert.equal(code, 0, out);
...
test("cell 21: loose objects outweighing the pack are named on the way past a sync", async () =>
{
    const { remoteBox, ws: synced } = await syncedWorkspace();
    const { code, out } = await selfIn(remoteBox, synced, ["sync"]);
    assert.equal(code, 0, out);
    assert.match(out, /store history: \d+ loose objects \([^)]+\) against a [^—]+pack — run `self store compact`/);
});

test("cell 22: a packed store says nothing about its history when it syncs", async () =>
{
    const { remoteBox, ws: synced } = await syncedWorkspace();
    await must(remoteBox, synced, ["store", "compact"]);
    const { code, out } = await selfIn(remoteBox, synced, ["sync"]);
    assert.equal(code, 0, out);
    assert.doesNotMatch(out, /store history:/);
});

test("cell 23: a store whose repository has no commit compacts as a no-op rather than an error", async () =>
{
    const fresh = machine();
    const made = await demoWorkspace(fresh);
    ...
    const { code, out } = await selfIn(fresh, made.demo, ["store", "compact"]);
    assert.equal(code, 0, out);
    assert.match(out, /history compacted/);
});

test("cell 24: a machine with no git says why compaction could not run, and refuses", async () =>
{
    const { code, out } = await selfIn(box, demo, ["store", "compact"], { PATH: join(box.root, "no-tools") });
    assert.equal(code, 1, out);
    assert.match(out, /store compact failed/);
});

test("cell 25: an unreachable loose object keeps git's own grace through a compaction", async () =>
{
    const blob = storeGit(box, store, ["hash-object", "-w", fileAt("cell25.txt", "cell 25 unreachable bytes\n")]);
    assert.match(blob, /^[0-9a-f]{40,64}$/);
    await must(box, demo, ["store", "compact"]);
    assert.doesNotThrow(() => storeGit(box, store, ["cat-file", "-e", blob]),
        "compaction destroyed an unreachable loose object");
});

test("cell 26: an unreachable object inside a previous pack survives compaction too", async () =>
{
...
test("cell 27: a store whose index is locked compacts without leaving the repository inconsistent", async () =>
{
    const lock = join(store, ".git", "index.lock");
    writeFileSync(lock, "");
    const compacted = await selfIn(box, demo, ["store", "compact"]);
    ...
    const after = await attach(["--artifact", fileAt("after.md", "cell 27 bytes\n")]);

...
import { connectedMachine, eventually, queueAppend, storeDir, syncEnv, unsent } from "./transport-lib.mjs";
...
test("rewrite crash-safe: the original stands until the rename, so a crash costs nothing", () =>
{
    ...
    writeFileSync(file, "the original\n");
    const nonce = acquireSyncLock(store);
    // A temp file left behind by a process that died before its rename. The
    // next holder writes its own, named by its own nonce, and this one is never
    // read by anything.

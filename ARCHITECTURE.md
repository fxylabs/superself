...
| Person present | `human.ts` `personAtTerminal`, and the refusal `personRefusal` mints from it | the one answer to "is there a person behind this process": an attempt marker the runner stamped, or a stdin that is not a terminal, means no. Every gate that needs a person reads it — the four that then read a typed challenge (`retirement.ts`, `report confirm`, `artifact prune`, `runbook approve`) and the two that need only presence, `work add` and `work accept` (#389). Presence and confirmation are different questions: a verb that prints a challenge and reads it back needs stdout as well and asks for both itself, while a presence gate prints nothing and reads stdin alone — so a person who pipes their own output is still a person |
